/*
 * @(#)ThreadSuspend.java	1.8 06/10/10
 *
...
class ThreadSuspend extends Thread {
	...
	for (int i = 0; i < threads; ++i) {
	    threadArray[i] = new ThreadSuspend(delay);
	    threadArray[i].start();
	}
	...
	while (true) {
	    ...
	    int i = r.nextInt(threads);
	    if (verbose) {
		System.err.println("ThreadSuspend suspending thread " + i);
	    }
	    /*threadArray[i].suspend();
	    if (verbose) {
		System.err.println("ThreadSuspend performing GC");
	    }
	    */
	    System.gc();
	    if (verbose) {
		System.err.println("ThreadSuspend resuming thread " + i);
	    }
	    /*
    ...
    public ThreadSuspend(long delay) {
	this.delay = delay;
    }
