...
public abstract class ClassLoader {
    ...
    */
    ...
    /*
     * The global root that points to this ClassLoader and will be stored
     * in the CVMcbClassLoader() field of every class loaded by this
     * ClassLoader. This way we don't need to allocate a separate global root
     * for each class loaded, and it also makes checking if two classes
     * have the same class loader a lot easier because we can just compare
     * the two CVMcbClassLoader() fields rather than having to become
     * gcsafe and compare what the two CVMcbClassLoader() fields refer to.
     * The global root is allocated by classcreate.c the first time
     * this ClassLoader loads a class.
     */
	...
	if (CVM.checkDebugFlags(CVM.DEBUGFLAG_TRACE_CLASSLOADING) != 0) {
	    System.err.println("CL: addClass() called for <" + 
			       c + "," + this + ">");
	}
...

    ...
    /*
     * Called by the constructor to setup the loaderGlobalRoot
	...
	/* load superclases in a way that avoids C recursion */
	c.loadSuperClasses();
    ...
    private static native Class loadBootstrapClass0(String name)
	throws ClassNotFoundException;
...

	...
	// the loader this native library belongs.
        ...
        String name;
        /* Indicates if the native library was loaded by a -Xrun option: */
        boolean isXrunLibrary;
	/* Indicates if the native library is linked with cvm */
	boolean isBuiltin;
	    ...
	    synchronized (loadedLibraryNames) {
                if ((isXrunLibrary || fromClass.getClassLoader() != null)
                    && handle != 0) {
                    if (!isXrunLibrary) {
    ...
    // Native libraries loaded by the -Xrun command-line option:
    ...
    private static Object loadLibraryInternal(Class fromClass, String name,
                                              boolean isAbsolute,
                                              boolean isXrunLibrary) {
	Object libObj;
	...
	sys_paths = CVM.getSystemLibrarySearchPaths();
        ...
        // Attempt to load the library from the absolute path if applicable:
        if (isAbsolute) {
        ...
        // Give the ClassLoader a chance to specify where to load the native
        // library from:
	if (loader != null) {
    ...
    "ClassLoader.findLibrary failed to return an absolute path: " + libfilename);
        ...
        // Check if the requested native library is amongst the statically
        // linked libraries:
	/*
	 * CVM statically links in some jni libraries that JDK
	 * dynamically links in.  For example, the net, math, and zip
	 * JNI libraries are all statically linked in. In order to prevent
	 * loadLibrary() of these libraries from failing, we need a
	 * mechanism for indicating which libraries are built in.
	 * We set java.library.builtin.<name> for each library that
	 * is statically built in. Licensees can also set properties
	...
	if (builtin != null) {
	    //System.err.println("Built-in library " + name + " found," +
	    //		       " java.library.builtin." + name);
	    libraryFound = true;
	} else {
	    String[] builtins = CVM.getBuiltinLibrarySearchPaths();
	    //System.err.println("Trying java.library.builtins: ");
	    for (int i = 0 ; i < builtins.length ; i++) {
		//System.err.println("\t" + builtins[i]);
		if (name.equals(builtins[i])) {
		    //System.err.println("Built-in library " + name +
		    //		       " found in java.library.builtins");
		    libraryFound = true;
		    break;
		}
	    }
	...
	for (int i = 0 ; i < sys_paths.length ; i++) {
        ...
        // Search java.library.path for the native library:
    ...
    private static Object loadLibrary0(Class fromClass, String name,
                                       boolean builtin, boolean isXrunLibrary)
    {
        // Get the classloader:
        ClassLoader loader = 
	    (fromClass == null) ? null : fromClass.getClassLoader();
        // Get the native library list for the specified classloader:
        Vector libs;
        if (isXrunLibrary) {
	...
	synchronized (libs) {
            // Search the classloader's native library list for a library with
            // the specified name:
	    int size = libs.size();
	    for (int i = 0; i < size; i++) {
	    ...
	    synchronized (loadedLibraryNames) {
                // Search the global native library list for a library with
                // the specified name to ensure that this library hasn't been
                // loaded yet:
		...
		for (int i = 0; i < n; i++) {
		    NativeLibrary lib = (NativeLibrary)
		        nativeLibraryContext.elementAt(i);
                    if (name.equals(lib.name) &&
                        (lib.isXrunLibrary == isXrunLibrary)) {
                ...
                // If we get here, then no one has loaded this native library
                // before.  Load the native library:
                NativeLibrary lib =
                    new NativeLibrary(fromClass, name, isXrunLibrary, builtin);
		if (!builtin) {
	...
	synchronized (libs) {
	    ...
	    for (int i = 0; i < size; i++) {
	        NativeLibrary lib = (NativeLibrary)libs.elementAt(i);
		/* No need to check builtin libraries since any symbols in
		 * them would already have been found by CVMfindBuiltinEntry().
		 */
		if (lib.isBuiltin) {
		    continue;
		}
