/*
 * @(#)DMArrayRWDoubleTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayRWDoubleTest.h"


JNIEXPORT void JNICALL
Java_DMArrayRWDoubleTest_nSetArray(JNIEnv *env, jobject obj, 
                          jdoubleArray dstArray, jdoubleArray srcArray, jint arrayLength)
{
   jint i;
   jdouble val;
   jobject lock;
   ...
   CVMD_gcUnsafeExec( ee, {
      CVMArrayOfDouble* directSrcArray = (CVMArrayOfDouble *)CVMID_icellDirect(ee, srcArray);
      CVMArrayOfDouble* directDstArray = (CVMArrayOfDouble *)CVMID_icellDirect(ee, dstArray);
      jint rBarrier = 0;
      jint wBarrier = 0;
     ...
     for(i=0; i<arrayLength; i++) {
      CVMD_arrayReadDouble(directSrcArray, i, val);
      if(ee->barrier == R_BARRIER_64)
         rBarrier++;
      CVMD_arrayWriteDouble(directDstArray, i, val);
      if(ee->barrier == W_BARRIER_64)
         wBarrier++;
     }

      if(rBarrier == arrayLength && wBarrier == arrayLength)
         printf("PASS: DMArrayRWDoubleTest, Read & Write Barrier Double were invoked\n");
      else
         printf("FAIL: DMArrayRWDoubleTest, Read & Write Barrier Double were not invoked\n");

      printf("\n");
       
      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayRWDoubleTest, Gc did not happen in the gc unsafe section\n");
      else
         printf("FAIL: DMArrayRWDoubleTest, Gc happened in the gc unsafe section\n");
   } );
}
