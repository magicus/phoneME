/*
 * @(#)ImageRepresentation.h	1.11 06/10/10
 *
...
typedef struct
{
	/* The pixmap that represents this image. A pixmap is an offscreen image on the server side. */
	...
	/* This is a temporary buffer used to store the RGB values for pixels that are set
	   using the setPixels method. It is used to reduce the number of times memory is allocated and freed
	   as the set pixels method is usually called many times (once for each line of image read in).
	   The pixel values are stored in this buffer as a sequence of red, green, blue values as bytes.
	   A pixbuf is then created from this buffer to be drawn into the pixmap. */
...
extern ImageRepresentationData *awt_gtk_getImageRepresentationData (JNIEnv *env, jobject this, jobject background);
