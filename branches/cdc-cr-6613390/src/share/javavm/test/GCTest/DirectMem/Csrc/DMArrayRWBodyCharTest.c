/*
 * @(#)DMArrayRWBodyCharTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayRWBodyCharTest.h"


JNIEXPORT void JNICALL
Java_DMArrayRWBodyCharTest_nSetArray(JNIEnv *env, jobject obj, 
                      jcharArray dstArray, jcharArray srcArray, jint arrLen)
{
   jint i;
   CVMJavaChar  buf[arrLen];
   jobject lock;
   jfieldID iFid, oFid;
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      CVMD_arrayReadBodyChar(buf, dSrcCharArray, 0, arrLen);
      if(ee->barrier == R_BARRIER_CHAR)
         printf("PASS: DMArrayRWBodyCharTest:Read, Read Barrier Char was invoked\n");
      else
         printf("FAIL: DMArrayRWBodyCharTest:Read, Read Barrier Char was not invoked\n");

      CVMD_arrayWriteBodyChar(buf, dDstCharArray, 0, arrLen);
      if(ee->barrier == W_BARRIER_CHAR)
         printf("PASS: DMArrayRWBodyCharTest:Write, Write Barrier Char was invoked\n");
      else
         printf("FAIL: DMArrayRWBodyCharTest:Write, Write Barrier Char was not invoked\n");

      printf("\n");

      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayRWBodyCharTest, Gc did not happen in the gc unsafe section\n");
      else
        printf("FAIL: DMArrayRWBodyCharTest, Gc happened in the gc unsafe section\n");
