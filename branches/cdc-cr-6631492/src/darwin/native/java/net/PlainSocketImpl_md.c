/*
 * @(#)PlainSocketImpl_md.c	1.18 06/10/10
 *
...
/* FIXME do we need it any longer
...
Java_java_net_PlainSocketImpl_socketCreate(JNIEnv *env, jobject this,
	...
	CHECK_NULL(JNI_STATIC_MD(java_net_PlainSocketImpl, socketExceptionCls));
    }
    ...
    if (ssObj != NULL) {
	int arg = 1;
        JVM_SetSockOpt(fd, SOL_SOCKET, SO_REUSEADDR, (char*)&arg, sizeof(arg));
    }
}
					   ...
					   jobject socket)
{
     ...
     * we can find the class InternalError in the java package, and
...
Java_java_net_PlainSocketImpl_socketSetOption(JNIEnv *env, jobject this,
	...
	case java_net_SocketOptions_IP_TOS :
		...
		if (cmd == java_net_SocketOptions_SO_RCVBUF) {
		    /* The darwin limit is 65535, or the call will fail */
		    if (optval.i > 65535) {
			optval.i = 65535;
		    }
		}
...
Java_java_net_PlainSocketImpl_socketGetOption(JNIEnv *env, jobject this,
    ...
    switch (cmd) {
	case java_net_SocketOptions_SO_LINGER:
	    /* The maximum we ever set SO_LINGER to is 0x0000ffff. However,
	     * darwin returns this as 0xffffffff. It seems to convert it to a
	     * signed short and then sign extends into an int. Because of
	     * this, we need to mask off the upper 16-bits.
	     */ 
	    optval.ling.l_linger &= 0xffff;
