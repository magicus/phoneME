/*
 * @(#)commProtocol_md.c	1.13 06/10/10
 * 
...
long openPortByName(char** ppszError, char* pszDeviceName, long baudRate,
    ...
    /* Need to prepend /dev/ before the devicename, if it is not */
    const char * prefix = "/dev/";

    /* do not become the controlling tty */
    openFlags = O_RDWR | O_NOCTTY;

    if (strncmp(pszDeviceName, prefix, strlen(prefix)) != 0) {
        char dest[strlen(pszDeviceName) + strlen(prefix) + 1];
        strcpy(dest, prefix);
        strcat(dest, pszDeviceName);
        hPort = open(dest, openFlags);
    } else {
...
/*  The following is not supported by Linux */
    ...
    /* 6227981: set it to NULL only if the error string was previously allocated */
    if (ppszError != NULL)
        *ppszError = NULL;
							  ...
							  jint flags)
{
  ...
  portNum = openPortByName(&errorMsg, (char *)portname, (long)baud,
                    (long)flags);
