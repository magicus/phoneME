...
echo ""
echo "Collecting Results in GenerationGC Test"
echo "---------------------------------------"
echo ""
...
then
	...
	then
		echo "GenerationGC Test build FAILED"
		echo "For more detail look in build.log"
	else
		echo "GenerationGC Test build PASSED"
	fi
else
...
then
	...
	then
		echo "GenerationGC Test run PASSED"
	fi

	if [ $FAIL -ne 0 ]
	then
		echo "GenerationGC Test run FAILED"
		echo ""
	fi

	if [ $UNRESOLVED -ne 0 ]
	then
		echo "GenerationGC Test run has UNRESOLVED test cases"
		echo ""
	fi
