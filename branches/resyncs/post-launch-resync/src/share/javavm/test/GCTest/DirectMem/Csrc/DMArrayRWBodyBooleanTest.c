/*
 * @(#)DMArrayRWBodyBooleanTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayRWBodyBooleanTest.h"


JNIEXPORT void JNICALL
Java_DMArrayRWBodyBooleanTest_nSetArray(JNIEnv *env, jobject obj, 
                   jbooleanArray dstArray, jbooleanArray srcArray, jint arrLen)
{
   jint i;
   CVMJavaBoolean  buf[arrLen];
   jobject lock;
   jfieldID iFid, oFid;
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      CVMD_arrayReadBodyBoolean(buf, dSrcBoolArray, 0, arrLen);
      if(ee->barrier == R_BARRIER_BOOLEAN)
         printf("PASS: DMArrayRWBodyBooleanTest:Read, Read Barrier Boolean was invoked\n");
      else
         printf("FAIL: DMArrayRWBodyBooleanTest:Read, Read Barrier Boolean was not invoked\n");

      CVMD_arrayWriteBodyBoolean(buf, dDstBoolArray, 0, arrLen);
      if(ee->barrier == W_BARRIER_BOOLEAN)
         printf("PASS: DMArrayRWBodyBooleanTest:Write, Write Barrier Boolean was invoked\n");
      else
         printf("FAIL: DMArrayRWBodyBooleanTest:Write, Write Barrier Boolean was not invoked\n");

      printf("\n");
       
      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayRWBodyBooleanTest, Gc did not happen in the gc unsafe section\n");
      else
        printf("FAIL: DMArrayRWBodyBooleanTest, Gc happened in the gc unsafe section\n");
