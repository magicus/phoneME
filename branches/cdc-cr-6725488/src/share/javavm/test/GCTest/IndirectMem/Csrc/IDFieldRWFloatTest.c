/*
 * @(#)IDFieldRWFloatTest.c	1.8 06/10/10
 *
...
#include <stdio.h>
#include "IDFieldRWFloatTest.h"

JNIEXPORT jfloatArray JNICALL 
Java_IDFieldRWFloatTest_nGetValues (JNIEnv *env, jobject obj) 
{
   jint i, j;
   jint numFields = 0;
   jfloat var;
      ...
      CVMD_gcSafeExec(ee, {
         ...
         for(i=0, j=0; i< CVMcbFieldCount(cb); i++) {
            ...
            if(!CVMfbIs(fb, STATIC)) {
               ...
               CVMID_fieldReadFloat(ee, (CVMObjectICell*) obj, objOffset, var);
               CVMID_arrayWriteFloat(ee, (CVMArrayOfFloatICell*) floatArray, i, var);
               ++j;
            }
         }
...
Java_IDFieldRWFloatTest_nSetValues(JNIEnv *env, jobject obj, 
                       jfloat v1, jfloat v2, jfloat v3)
{
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      CVMD_gcSafeExec(ee, {
         CVMID_fieldWriteFloat(ee, (CVMObjectICell*) obj, objOffset[0], v1);
         CVMID_fieldWriteFloat(ee, (CVMObjectICell*) obj, objOffset[1], v2);
         CVMID_fieldWriteFloat(ee, (CVMObjectICell*) obj, objOffset[2], v3);
