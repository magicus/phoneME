/*
 * @(#)IDFieldRWDoubleTest.c	1.8 06/10/10
 *
...
#include <stdio.h>
#include "IDFieldRWDoubleTest.h"

JNIEXPORT jdoubleArray JNICALL 
Java_IDFieldRWDoubleTest_nGetValues (JNIEnv *env, jobject obj) 
{
   jint i, j;
   jint numFields = 0;
   jclass clazz;
      ...
      CVMD_gcSafeExec(ee, {
         ...
         for(i=0, j=0; i< CVMcbFieldCount(cb); i++) {
            ...
            if(!CVMfbIs(fb, STATIC)) {
               ...
               CVMID_fieldReadDouble(ee, (CVMObjectICell*) obj, objOffset, var);
               CVMID_arrayWriteDouble(ee, (CVMArrayOfDoubleICell*) doubleArray, i, var);
               ++j;
            }
         }
...
Java_IDFieldRWDoubleTest_nSetValues(JNIEnv *env, jobject obj, 
                       jdouble v1, jdouble v2, jdouble v3)
{
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      CVMD_gcSafeExec(ee, {
         CVMID_fieldWriteDouble(ee, (CVMObjectICell*) obj, objOffset[0], v1);
         CVMID_fieldWriteDouble(ee, (CVMObjectICell*) obj, objOffset[1], v2);
         CVMID_fieldWriteDouble(ee, (CVMObjectICell*) obj, objOffset[2], v3);
