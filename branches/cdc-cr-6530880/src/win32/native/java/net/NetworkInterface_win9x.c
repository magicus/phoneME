/*
 * @(#)NetworkInterface_win9x.c	1.9 06/10/10
 *
...
static int isWanDriver(char *driver) {
    ...
    DWORD dwLen;
    ULONG ulType;
    TCHAR key[MAX_STR_LEN];
    char vxd[MAX_STR_LEN];

    wsprintf(key, _T("System\\CurrentControlSet\\Services\\Class\\%hs"), driver);
    ret = RegOpenKeyEx(HKEY_LOCAL_MACHINE, key, 0, KEY_READ, (PHKEY)&hKey);
    if (ret != ERROR_SUCCESS) {
	return 0;
    }
    dwLen = sizeof(vxd);
    ret = RegQueryValueEx(hKey, _T("DeviceVxDs"),  NULL, &ulType,
                         (LPBYTE)vxd, &dwLen);
    RegCloseKey(hKey);
    if (ret != ERROR_SUCCESS) {
	return 0;
...
static int getAdapters(JNIEnv *env, adapter **adapterPP)
{
     ...
     */
    ret = RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("Enum"), 0, KEY_READ, (PHKEY)&enumKey);
    if (ret != ERROR_SUCCESS) {
	return -1;
    }
	...
	for (deviceIndex=0; deviceIndex<dwDeviceKeys; deviceIndex++) {
		...
		if (ret == ERROR_SUCCESS) {
		    dwLen = sizeof(cls);
		    ret = RegQueryValueEx(clsKey, _T("Class"),  NULL, &ulType,
                                          (LPBYTE)cls, &dwLen);

		    if (ret == ERROR_SUCCESS) {
			if (_tcscmp(cls, _T("Net")) == 0) {
			    char deviceDesc[MAX_STR_LEN];

			    dwLen = sizeof(deviceDesc);
			    ret = RegQueryValueEx(clsKey, _T("DeviceDesc"),  NULL, &ulType,
                                                  (LPBYTE)deviceDesc, &dwLen);

			    if (ret == ERROR_SUCCESS) {	
				TCHAR key_name[MAX_STR_LEN];
				char ps_name[8];
				 ...
				 */
				dwLen = sizeof(driver);
				ret = RegQueryValueEx(clsKey, _T("Driver"),  NULL, 
				 ...
				 */
				wsprintf(key_name, _T("Enum\\%hs\\%hs\\%hs"), deviceType, name, dev);
				...
				if (curr != NULL) {				    				
					...
					if (curr->displayName) {					    
					    curr->reg_key = (TCHAR *)malloc((_tcslen(key_name) + 1) * sizeof(TCHAR));
					    if (curr->reg_key == NULL) {
						free(curr->displayName);
						free(curr->name);
						free(curr);
 ...
 */
static int getStaticAddressEntry(TCHAR *binding, char *addresses) {
    LONG ret;
    HKEY hKey;
    TCHAR name[255];
    char desc[255];
     ...
     */
    wsprintf(name, _T("Enum\\Network\\%s"), binding);
    ret = RegOpenKeyEx(HKEY_LOCAL_MACHINE, name, 0, KEY_READ, (PHKEY)&hKey);
    if (ret != ERROR_SUCCESS) {
	return -1;
    }
    dwName = sizeof(desc);
    ret = RegQueryValueEx(hKey, _T("DeviceDesc"),  NULL, &ulType,
                         (LPBYTE)desc, &dwName);
    if (ret != ERROR_SUCCESS) {
	RegCloseKey(hKey);
	return -1;
    ...
    ret = RegQueryValueEx(hKey, _T("Driver"),  NULL, &ulType,
     ...
     */
    wsprintf(name, _T("System\\CurrentControlSet\\Services\\Class\\%hs"), driver);
    ret = RegOpenKeyEx(HKEY_LOCAL_MACHINE, name, 0, KEY_READ, (PHKEY)&hKey);
    if (ret != ERROR_SUCCESS) {
	return -1;
    }
    dwName = sizeof(ipaddr);
    ret = RegQueryValueEx(hKey, _T("IPAddress"),  NULL, &ulType,
 ...
 */
static int getStaticAddresses(JNIEnv *env, TCHAR *reg_key, netaddr **netaddrPP) 
{
    LONG ret;
	...
	*netaddrPP = NULL;
     ...
     * and any static address associated with the binding.
     */
    strcpy(addresses, "");
    addrCount = 0;
    netaddrP = NULL;

    ret = RegOpenKeyEx(enumKey, _T("Bindings"), 0, KEY_READ, (PHKEY)&bindingKey);
    if (ret == ERROR_SUCCESS) {				   
	DWORD dwBindingKeys;
...
static DWORD getDHCPAddress()
{
	...
	wsprintf(key, _T("SYSTEM\\CurrentControlSet\\Services\\VxD\\DHCP\\DhcpInfo%02d"), index);
	 ...
	 */
	dwLen = sizeof(addr);
	ret = RegQueryValueEx(hKey, _T("DhcpIPAddress"),  NULL, &ulType,
			      (LPBYTE)&addr, &dwLen);
	RegCloseKey(hKey);
...
static adapter *loadConfig(JNIEnv *env) {
	...
	while (adapterP != NULL) {
		...
		while (addrP != NULL) {
		    if (addrP->addr == static_addrP->addr)
			break;
