 ...
 * a subsetted version for CDC;
 * this subsetted version for CDC/FP; and
 * a more complete version for the security optional package.
 * Make sure you're editting the correct one!
 */
...
// Subset out PrincipalComparator references for CDC/FP
//import com.sun.security.auth.PrincipalComparator;
import java.security.*;
import java.security.cert.Certificate;
import java.security.cert.X509Certificate;
// Subset out javax.security.auth references for CDC/FP,
// with the exception of X500Principal.
...
import java.util.PropertyPermission;
// Subset out AWTPermission reference for CDC/FP.
...
public class PolicyFile extends java.security.Policy {
	...
	initPolicyFile(newInfo);
	policyInfo = newInfo;
    }

    private void initPolicyFile(PolicyInfo newInfo) {
	...
	// To maintain strict backward compatibility we load the static policy
	// only if POLICY load failed
	if (!loaded_one) {
	...
	try {
	    ...
	    if (notUtf8) {
	        isr = new InputStreamReader(getInputStream(policy)); 
	    } else {
	        isr = new InputStreamReader(getInputStream(policy), "UTF-8"); 
	    }
	...
	AccessController.doPrivileged(new PrivilegedAction() {
		...
		try {
		    File f = new File(PropertyExpander.expand(
			"${java.home}/lib/ext/*"));
		    pe = new PolicyEntry(
			new CodeSource(ParseUtil.fileToEncodedURL(f), null));
...
*/
	...
	Boolean imp = (Boolean) 
		...
		public Object run() {
		    return new Boolean(
			entry.getCodeSource().implies(cs));
		}
	    });
	...
	if (! imp.booleanValue()) {
		...
		for (int j = 0; j < pdp.length; j++) {
		    pe = new PolicyParser.PrincipalEntry
			(pdp[j].getClass().getName(),
			    pdp[j].getName());
		    domainPs.add(pe);
		}
	    }
		...
		ListIterator li = entryPs.listIterator(0); 
		while (li.hasNext()) {
		    PolicyParser.PrincipalEntry pppe =
			(PolicyParser.PrincipalEntry)li.next();

                    /* Subset out PrincipalComparator references
                     * and java.security.auth classes in CDC/FP
		    try {
			// handle PrincipalComparators
			Class principalComparator = Class.forName
			    (pppe.principalClass, true,
				Thread.currentThread().getContextClassLoader());

			Constructor c =
			    principalComparator.getConstructor(PARAMS1);
			...
			// implies the current Subject in the ACC
			...
			if ((addPerms = pc.implies(subject)) == false) {
			    // immediately break if one principal
			    // in the policy does not imply the
			    // current Subject
			    break;
			}
			...
			if (debug != null) {
			    debug.println
				("Exception while attempting " +
				    "to construct PrincipalComparator");
			    e.printStackTrace();
			}

			if ((addPerms = subjectListImpliesPrincipalEntry
			    (domainPs, pppe)) == false) {
			    break;
			}
		    }
                    * end of CDC/FP subsetting.
                    */
		}
	    }
		...
		if (debug != null) {
		    debug.println
			("evaluation (codesource/principals) passed");
		}
		for (int j = 0; j < entry.permissions.size(); j++) {
		    Permission p = (Permission)
			entry.permissions.get(j);
		    if (debug != null) {
			debug.println("  granting " + p);
		    }
		    // handle "SELF" permissions
		    if (p instanceof SelfPermission) {
			expandSelf((SelfPermission)p, entryPs, pdp, perms);
		    } else {
			perms.add(p);
		    }
		}
     ...
     *          that correspond to all the Principals in the Subject
     *          currently on this thread's AccessControlContext. <p>
     *
     ...
     *          <i>pppe</i> argument.
     */
    private boolean subjectListImpliesPrincipalEntry(List subjectList,
                                        PolicyParser.PrincipalEntry pppe) {

        ListIterator li = subjectList.listIterator(0);
        while (li.hasNext()) {
            PolicyParser.PrincipalEntry listPppe =
	...
	while ((v = sp.getSelfName().indexOf(SELF, startIndex)) != -1) {
	    ...
	    // expand SELF 
	    ListIterator pli = entryPs.listIterator();
	    while (pli.hasNext()) {
		PolicyParser.PrincipalEntry pppe =
	...
	try {
	    ...
	    Class pc = null;
	    while (e.hasMoreElements()) {
		Permission pElement = (Permission)e.nextElement();
	...
	if (u != null) {
	   Permission p;
	   try {
		p = u.openConnection().getPermission();
	   ...
	   if (p instanceof FilePermission) {
		path = p.getName();
	   } else if ((p == null) && (u.getProtocol().equals("file"))) {
     ...
     */
    private boolean replacePrincipals(List principals, KeyStore keystore) {
	...
	while (i.hasNext()) {
	    PolicyParser.PrincipalEntry pppe =
				(PolicyParser.PrincipalEntry)i.next();
	...
	synchronized(PolicyFile.class) {
	    if (scope == null) {
		IdentityScope is = (IdentityScope)AccessController.doPrivileged
		    (new java.security.PrivilegedAction() {
		    public Object run() {
	...
	// to get a signer
	final Identity id = (Identity) AccessController.doPrivileged(
				      new java.security.PrivilegedAction() {
	    public Object run() {
...
class PolicyInfo {
	...
	identityPolicyEntries = new ArrayList(2);
	aliasMapping = new HashMap(11);

	pdMapping = new Map[numCaches];
	for (int i = 0; i < numCaches; i++) {
	    pdMapping[i] = Collections.synchronizedMap(new WeakHashMap());
	}
	if (numCaches > 1) {
