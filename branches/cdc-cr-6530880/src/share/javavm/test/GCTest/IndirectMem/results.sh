...
echo ""
echo "Collecting Results in IndirectMem"
echo "---------------------------------"
echo ""
...
then
	...
	then
		echo "IndirectMem build FAILED"
		echo "For more detail look in build.log"
	else
		echo "IndirectMem build PASSED"
	fi
else
...
then
	...
	then
		echo "IndirectMem run PASSED"
	fi

	if [ $FAIL -ne 0 ]
	then
		echo "IndirectMem run FAILED"
		echo ""
	fi

	if [ $UNRESOLVED -ne 0 ]
	then
		echo "IndirectMem run has UNRESOLVED test cases"
		echo ""
	fi
