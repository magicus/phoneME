/*
 * @(#)DMArrayCopyIntTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayCopyIntTest.h"


JNIEXPORT void JNICALL
Java_DMArrayCopyIntTest_nSetArray(JNIEnv *env, jobject obj, 
                                  jintArray srcArray, jintArray dstArray)
{
   jint i;
   jobject lock;
   jfieldID iFid, oFid;
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      CVMD_arrayCopyInt(dSrcIntArray, 0, dDstIntArray, 0, CVMD_arrayGetLength(dDstIntArray));

      if(ee->barrier == W_BARRIER_INT)
         printf("PASS: DMArrayCopyIntTest, Write Barrier Int was invoked\n");
      else
         printf("FAIL: DMArrayCopyIntTest, Write Barrier Int was not invoked\n");

      printf("\n");
       
      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayCopyIntTest, Gc did not happen in the gc unsafe section\n");
      else
         printf("FAIL: DMArrayCopyIntTest, Gc happened in the gc unsafe section\n");
