/*
 * @(#)DMArrayRWRefTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayRWRefTest.h"

JNIEXPORT void JNICALL
Java_DMArrayRWRefTest_nSetArray(JNIEnv *env, jobject obj, 
                  jobjectArray dstStringArray, jobjectArray srcStringArray, 
                  jint arrayLength)
{
   jint i;
   jobject lock;
   ...
   CVMD_gcUnsafeExec( ee, {
      CVMArrayOfRef* srcRefArray = (CVMArrayOfRef *)CVMID_icellDirect(ee, srcStringArray);
      CVMArrayOfRef* dstRefArray = (CVMArrayOfRef *)CVMID_icellDirect(ee, dstStringArray);
      ...
      for(i=0; i<arrayLength; i++) {
         CVMD_arrayReadRef(srcRefArray, i, stringDirect);
         if(ee->barrier == R_BARRIER_REF)
            rBarrier++;
         CVMD_arrayWriteRef(dstRefArray, i, stringDirect);
         if(ee->barrier == W_BARRIER_REF)
            wBarrier++;
      }

      if(rBarrier == arrayLength && wBarrier == arrayLength)
         printf("PASS: DMArrayRWRefTest, Read and Write Barrier were invoked\n");
      else
         printf("FAIL: DMArrayRWRefTest, Read and Write Barrier were not invoked\n");

      printf("\n");
       
      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayRWRefTest, Gc did not happen in the gc unsafe section\n");
      else
         printf("FAIL: DMArrayRWRefTest, Gc happened in the gc unsafe section\n");
   } );
