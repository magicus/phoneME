...
static void handleSegv(int sig, siginfo_t* info, void* ucpPtr)
{
    ...
    struct sigaction sa;
    CVMUint8* pc = (CVMUint8 *)ucp->uc_mcontext.gregs[REG_EIP];
    CVMUint8** sp = (CVMUint8**)ucp->uc_mcontext.gregs[REG_ESP];
...
#ifdef CVMJIT_TRAP_BASED_GC_CHECKS
	{
		...
		if (offset >= 0) {
		    ...
		    *(--sp) = pc - offset;
		    ucp->uc_mcontext.gregs[REG_ESP] = sp;
		     ...
		     * then reload phis.
		     */
		    ucp->uc_mcontext.gregs[REG_EIP] += offset;
		}
		return;
	    }
	}
