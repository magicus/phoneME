...
public class TLV {
    ...
    /**
     * Creates TLV object of type octet string.
     * @param p_data byte[] value
     * @param offset int offset in the p_data
     * @param length int  number of bytes from the p_data
     * @return new object
     */
    public static TLV createOctetString(byte[] p_data, int offset, int length) {
        byte[] data = new byte[length];
        for (int i = 0; i < length; i++) {
            data[i] = p_data[i + offset];
        }
        return new TLV(OCTETSTR_TYPE, data);
    }

    /**
     * Creates TLV object of type bit string.
     * @param p_data byte[] value
     * @param notUsed int number of bytes from the p_data that are not used
     * @return new TLV object
     */
    public static TLV createBitString(byte[] p_data, int notUsed) {
        byte[] data = new byte[p_data.length + 1];
        data[0] = (byte)notUsed;
        for (int i = 0; i < p_data.length; i++) {
            data[i + 1] = p_data[i];
        }
        return new TLV(BITSTRING_TYPE, data);
    }
