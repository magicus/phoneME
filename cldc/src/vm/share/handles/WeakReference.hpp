...
class WeakReference : public Instance {
 public:
  ...
  static int next_offset() {
    return header_size() + 1 * sizeof(jobject);
  }
  ...
  static int reference_queue_offset() {
    return header_size() + 2 * sizeof(jobject);
  }

  ReturnOop reference_queue() const {
    return obj_field(reference_queue_offset());
  }

  void set_reference_queue(Oop* value) {
    obj_field_put(reference_queue_offset(), value);
  }
