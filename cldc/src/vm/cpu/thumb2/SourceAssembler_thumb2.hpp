...
// Porting macros for ARM source assembler
#define imm_index(rn, offset) (rn), (offset)
#define imm_shift(rn, s, n)   (rn), s##_shift, (n)
#define reg_shift(rn, s, rm)  (rn), s##_shift, (rm)
...
class SourceAssembler: public Macros {
 ...
 protected:
  ...
  // disassembler
  virtual void emit(short instr);
  virtual void emit_int(int instr);
  virtual void emit_w(int instr);
 ...
 public:
  inline int imm(int n) {
    return n;
  }
  void add_imm(Register rm, int imm) {
    Macros::add(rm, rm, imm);
  }
  void sub_imm(Register rm, int imm) {
    Macros::sub(rm, rm, imm);
  }
  void add_imm(Register rm, Register rn, int imm, CCMode s = no_CC);
  void sub_imm(Register rm, Register rn, int imm, CCMode s = no_CC);

  // Porting API for ARM source assembler
#define F(mnemonic) \
  void mnemonic(Register rxf, Register rn, int offset = 0) { \
    if (rn < r8 && rxf < r8 && offset >= 0 && offset < 128) { \
      Macros::mnemonic(rxf, rn, offset); \
    } else { \
      Macros::mnemonic##_imm12_w(rxf, rn, offset); \
    } \
  }
  F(str)
  F(ldr)
#undef F
  ...
  class Label {
   private:
    ...
    void bind    (Stream* s, bool is_global = false, int code_type = THUMB_CODE);
 ...
 public:
  ...
  void ldr_gp_base(Register r, Register tmp, Condition cond = al) { 
    if (r < r8) {
      ldr_label(r, "gp_base_label", false, cond);
    } else {
      ldr_label(tmp, "gp_base_label", false, cond);
      mov(r, reg(tmp));
    }
  }
  ...
  void bind  (Label& L, int code_type = THUMB_CODE)
                                       { L.bind(stream(), false, code_type); }
  void bind_global(Label& L, int code_type = THUMB_CODE);
  void bind  (const char *name)        { Label L(name); bind(L); }
  void bind_global(const char *name, int code_type = THUMB_CODE)   
              { Label L(name); bind_global(L, code_type); }
  ...
  void b_w(const Label& L, Condition cond = al) {
    it(cond);
    b(L);
  }
  // data definition
  void define_byte (int x);
  void define_short(int x);
