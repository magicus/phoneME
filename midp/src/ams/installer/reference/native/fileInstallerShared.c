...
/* IMPL NOTE: these functions should be declared in appropriate header file */
extern int jsr211_verifyHandlers(MidpProperties jadsmp, MidpProperties mfsmp,
                                 void* jarHandle, jchar trusted);
extern int jsr211_store_handlers(const pcsl_string * suiteID,
                                 const pcsl_string * ca);
...
fileInstaller(int argc, char* argv[]) {
...
#ifdef ENABLE_JSR_211
    if (res >= 0) {
        if (jsr211_verifyHandlers(jadsmp, mfsmp, jarHandle, trusted) < 0) {
            res = -1;
        }
    }
#endif  /* ENABLE_JSR_211 */
    ...
    midp_get_suite_id(mf_vendor, mf_name, &suiteID);
    if (pcsl_string_length(&suiteID) > 0) {
        REPORT_ERROR(LC_AMS, 
    ...
    if (OUT_OF_MEM_LEN == midp_create_suite_id(mf_vendor, mf_name, &suiteID)) {
        REPORT_ERROR(LC_AMS, "Out Of Memory");
        if (usingWhat == USING_JAD) {
            midpFreeProperties(jadsmp);
            pcsl_string_free(&jadRelativeURL);
    ...
    do {
...
#ifdef ENABLE_JSR_211
        res = jsr211_store_handlers(&suiteID, &PCSL_STRING_NULL);
        if (res != 0) {
        ...
        res = midp_store_suite(&suiteID, /* unique ID of the suite */
                               &jadURL, /* where the JAD came from */
                               jadsmp, /* properties of the JAD */
                               &jarRelativeURL, /* where the JAR came from */
                               &TEMP_JAR_NAME, /* the downloaded JAR,
                                                  will be moved */
                               mfsmp, /* properties of the manifest */
                               &domain, /* domain of the suite */
                               trusted, /* true/false */
                               &AuthPath_tmp, /* certificate authorities,
                                                 if signed */
                               0, /* number of authorities */
                               permissions, /* permissions for the suite */
                               permLen, /* length of the permissions array */
                               8, /* push interrupt settings, session */
                               0, /* push options */
                               verifyHash, /* verify hash data */
                               verifyHashLen); /* verify hash length */
        if (res != 0) {
            /* something wrong */
