...
/**
 * Get a pcsl_string from a field of an object.
 *
 * returns PCSL_STRING_OK upon success, error code otherwise
 */
pcsl_string_status getStringField(jobject, jclass, char*, jobject,
                                    pcsl_string *);
...
#define KNI_SAVE_PCSL_STRING_FIELD(OBJECT,CLASS,NAME,VALUE,HANDLE) { \
    ...
    if (PCSL_STRING_ENOMEM == \
        getStringField((OBJECT), (CLASS), (NAME), (HANDLE), (VALUE))) { \
