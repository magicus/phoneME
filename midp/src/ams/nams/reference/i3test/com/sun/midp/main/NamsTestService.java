...
public class NamsTestService implements EventListener, Runnable {
    ...
    public static void init(SecurityToken token, EventQueue eq) {
        ...
        initialize(MIDletSuiteLoader.getIsolateId());
    ...
    void reply(String s) {
        ...
        // XXX not thread-safe
        // XXX if no connection, should buffer up events
    ...
    void processLine(String[] sa) {
        ...
        if ("quit".equals(sa[0])) {
            ...
            if (app != null) {
                NamsAPIWrapper.midletCreateStart(sa[1], sa[2],
                    app.intValue());
            }
    ...
    public void process(Event event) {
        ...
        switch (callbackId) {
        case 0:         // background
            reply("> background " + reason);
            break;
        case 1:         // foreground
            reply("> foreground " + appId + " " + reason);
            break;
        case 2:         // state change
            reply("> state " + appId + " " + state + " " + reason);
            break;
        default:
            reply(
