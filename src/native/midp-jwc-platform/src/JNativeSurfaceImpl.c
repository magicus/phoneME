...
Java_com_sun_pisces_NativeSurface_draw() {
    ...
    imdata idd;
    if (gr != NULL) {
        get_pixmap_from_midp_image((void *)gr->img, & idd);
    } else {
    ...
    if (surface != NULL && surface->super.data != NULL && idd.data != NULL
        && idd.d == 16 && opacity != 0) {
            for(yy = 0 ; yy < surface->super.height; yy++) {
                for(xx = 0; xx < surface->super.width; xx++) {
                    ...
                    sam = 255 - sa;
                    if (sa != 0) {
                        offset = (yy + y) * idd.w + xx + x;

                        srgb = *((int *) surface->super.data + soffset);
    
                        rgb = *((unsigned short *) idd.data + offset);
                        ...
                        *((unsigned short *) idd.data + offset) =                                                               ((dr & 0x1f) << 11) + ((dg & 0x3f) << 5) + (db & 0x1f);
                    }                        
                }
            }
    }
