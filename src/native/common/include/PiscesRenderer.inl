...
static INLINE Renderer* renderer_create(Surface* surface, jint imageType);
...
static INLINE void renderer_setAntialiasing(Renderer* rdr, jboolean antialiasingOn);
...
static INLINE void renderer_setTransform(Renderer* rdr, const Transform6* transform);
static INLINE Transform6* renderer_getTransform(Renderer* rdr);
static INLINE void renderer_setStroke0(Renderer* rdr);
static INLINE void renderer_setStroke6(Renderer* rdr, jint lineWidth, jint capStyle, 
    jint joinStyle, jint miterLimit, jint* dashArray, jint dashArray_length,
    jint dashPhase);
static INLINE void renderer_setFill(Renderer* rdr);
static INLINE void renderer_setColor(Renderer* rdr, jint red, jint green, jint blue, 
    jint alpha);
static INLINE void renderer_setLinearGradient(Renderer* rdr, jint x0, jint y0, jint x1, jint y1, jint* colors, Transform6 *transform);
static INLINE void renderer_setRadialGradient(Renderer* rdr, jint cx, jint cy, jint fx, jint fy, jint radius, jint* colors, Transform6 *transform);
static INLINE void renderer_setTexture(Renderer* rdr, jint* data, jint width, jint height,
...
static INLINE void renderer_quadTo(Renderer* rdr, jint x1, jint y1, jint x2, jint y2);
static INLINE void renderer_cubicTo(Renderer* rdr, jint x1, jint y1, jint x2, jint y2, 
    jint x3, jint y3);
static INLINE void renderer_close(Renderer* rdr);
static INLINE void renderer_end(Renderer* rdr);

static INLINE void renderer_clearRect(Renderer* rdr, jint x, jint y, jint w, jint h);
static INLINE void renderer_drawLine(Renderer* rdr, jint x0, jint y0, jint x1, jint y1);
static INLINE void renderer_drawRect(Renderer* rdr, jint x, jint y, jint w, jint h);
static INLINE void renderer_fillRect(Renderer* rdr, jint x, jint y, jint w, jint h);
static INLINE void renderer_drawOval(Renderer* rdr, jint x, jint y, jint w, jint h);
static INLINE void renderer_fillOval(Renderer* rdr, jint x, jint y, jint w, jint h);
static INLINE void renderer_drawRoundRect(Renderer* rdr, jint x, jint y, jint w, jint h, 
    jint aw, jint ah);
static INLINE void renderer_fillRoundRect(Renderer* rdr, jint x, jint y, jint w, jint h, 
    jint aw, jint ah);
static INLINE void renderer_drawArc(Renderer* rdr, jint x, jint y, jint width, jint height, 
    jint startAngle, jint arcAngle, jint arcType);
static INLINE void renderer_fillArc(Renderer* rdr, jint x, jint y, jint width, jint height, 
    jint startAngle, jint arcAngle, jint arcType);

static Renderer* createCommon(jint imageType);
static void setPaintMode(Renderer* rdr, jint newPaintMode);
static void createAlphaMap(Renderer* rdr);
...
static void emitOffsetOval(Pipeline* pipeline, jint cx, jint cy, 
    jint rx, jint ry, jint lw2, jint nPoints, jboolean inside);
static void fillOrDrawOval(Renderer* rdr, jint x, jint y, jint w, jint h, 
    jboolean hollow);
static void emitRoundRect(Pipeline* pipeline, jint x, jint y, 
    jint w, jint h, jint aw, jint ah, jboolean reverse);
...
static void fillOrDrawArc(Renderer* rdr, jint x, jint y, 
    jint width, jint height, jint startAngle, jint arcAngle, 
    jint arcType, jboolean stroke);

static INLINE Renderer* 
renderer_create(Surface* surface, jint imageType) {
  Renderer* rdr = createCommon(imageType);
  if (rdr == NULL) {
    return NULL;
  }

  rdr->_data = rdr->_intData = surface->data;
  rdr->_imageOffset = 0;
  rdr->_imageScanlineStride = surface->width;
  rdr->_imagePixelStride = 1;
  rdr->_allocated = XNI_FALSE;

  rdr->_width = surface->width;
  rdr->_height = surface->height;
...
renderer_beginRendering1(Renderer* rdr, jint windingRule) {
  jint minX = MAX(0, rdr->_clip_bbMinX);
  jint minY = MAX(0, rdr->_clip_bbMinY);
  jint maxX = MIN(rdr->_width, rdr->_clip_bbMaxX);
  jint maxY = MIN(rdr->_height, rdr->_clip_bbMaxY);
  renderer_beginRendering5(rdr, minX, minY, maxX - minX, maxY - minY, 
      windingRule);
}
...
renderer_setColor(Renderer* rdr, jint red, jint green, jint blue, jint alpha) {
  if (rdr->_imageType == TYPE_INT_RGB || rdr->_imageType == TYPE_INT_ARGB || 
      rdr->_imageType == TYPE_BYTE_GRAY) {
    rdr->_cred = red;
    rdr->_cgreen = green;
    rdr->_cblue = blue;
  } else {
    rdr->_cred = _Pisces_convert8To5[red];
    rdr->_cgreen = _Pisces_convert8To6[green];
    rdr->_cblue = _Pisces_convert8To5[blue];
  }
  rdr->_calpha = alpha;
  createAlphaMap(rdr);
...
createCommon(jint imageType) {
  ...
  rdr->_imageType = imageType;

  switch(imageType) {
          case TYPE_INT_RGB:
            rdr->_bl_SO = blitSrcOver888;
            rdr->_bl_PT_SO = blitPTSrcOver888;
            rdr->_clearRect = clearRect8888;
            break;
          case TYPE_INT_ARGB:
            rdr->_bl_SO = blitSrcOver8888;
            rdr->_bl_PT_SO = blitPTSrcOver8888;
            rdr->_clearRect = clearRect8888;
            break;
          case TYPE_USHORT_565_RGB:
            rdr->_bl_SO = blitSrcOver565;
            rdr->_bl_PT_SO = blitPTSrcOver565;
            rdr->_clearRect = clearRect565;
            break;
          case TYPE_BYTE_GRAY:
            rdr->_bl_SO = blitSrcOver8;
            rdr->_bl_PT_SO = blitPTSrcOver8;
...
static void
createAlphaMap(Renderer* rdr) {
  jint i;
  for (i = 0; i <= rdr->_MAX_AA_ALPHA; i++) {
    rdr->_alphaMap[i] = (256*i*rdr->_calpha + rdr->_HALF_MAX_AA_ALPHA_DENOM)/
...
setAntialiasing(Renderer* rdr, jint subpixelLgPositionsX, 
  ...
  rdr->_MAX_AA_ALPHA = (rdr->_SUBPIXEL_POSITIONS_X*rdr->_SUBPIXEL_POSITIONS_Y);
...
computeCrossingsForEdge(Renderer *rdr, jint index, 
  ...
  addCrossing(rdr, y >> rdr->_YSHIFT, (jint)(lx >> rdr->_XSHIFT), orientation);
  /* Check for error in memory allocation */
  if (KNI_TRUE == readMemErrorFlag()) {
    return;
  }
...
renderStrip(Renderer* rdr) {
  ...
  width =
    (rdr->_rasterMaxX - rdr->_rasterMinX + 1) >> rdr->_SUBPIXEL_LG_POSITIONS_X;
  ...
  while (hasMoreCrossingRows(rdr)) {
    ...
    if (rdr->_crossingRowIndex < rdr->_crossingRowCount) {
      ...
      lx = rdr->_crossings[rdr->_crossingRowOffset + rdr->_crossingRowIndex];
      lx >>= 1;
      hx = rdr->_crossings[rdr->_crossingRowOffset + rdr->_crossingRowCount - 1];
      hx >>= 1;
      ...
      if ((sum & mask) != 0) {
        ...
        if (x1 > x0) {
          ...
          if (x == xmaxm1) {
             ...
             *tmpRowAA++ += rdr->_SUBPIXEL_POSITIONS_X - (x0 & rdr->_SUBPIXEL_MASK_X);
             x++;
             ...
             *tmpRowAA += x1 & rdr->_SUBPIXEL_MASK_X;
             ...
             jint count;
             jint rowAdd = (rdr->_SUBPIXEL_POSITIONS_X << 24) | (rdr->_SUBPIXEL_POSITIONS_X << 16) | (rdr->_SUBPIXEL_POSITIONS_X << 8) | rdr->_SUBPIXEL_POSITIONS_X;                 
                          
             jbyte *tmpRowAA = &rdr->_rowAA[rdr->_rowAAOffset + x];
                          
             /* Have to record this as we could clobber the last byte with the int writes */
             jbyte tmpAA = tmpRowAA[xmax-x];
                          
             *tmpRowAA++ += rdr->_SUBPIXEL_POSITIONS_X - (x0 & rdr->_SUBPIXEL_MASK_X);
             x++;
                          
             /* assume as there are at least 8 values this won't overrun */
...
validateStroker(Renderer* rdr) {
  if (!rdr->_validStroker) {
    pipeline_setStrokerParameters(&rdr->_strokerPipeline,
        rdr->_lineWidth, rdr->_capStyle, rdr->_joinStyle,
        rdr->_miterLimit, rdr->_dashArray, rdr->_dashArray_length,
        rdr->_dashPhase, &rdr->_transform);
...
emitOffsetOval(Pipeline* pipeline, jint cx, jint cy, jint rx, jint ry,
  ...
  for (j = 0; j < nPoints; j++) {
    ...
    jdouble den = dlw2/PISCESsqrt(drxSinTheta*drxSinTheta + dryCosTheta*dryCosTheta);
...
fillOrDrawOval(Renderer* rdr, jint x, jint y, jint w, jint h, jboolean hollow) {
  ...
  if (hollow) {
    ...
    if (wl > 0 && hl > 0) {
      if (w == h) {
        emitOval(&rdr->_fillerPipeline, cx, cy, wl, hl, nPoints, XNI_TRUE);
      } else {
        emitOffsetOval(&rdr->_fillerPipeline, cx, cy, w2, h2, lineWidth2,
            nPoints, XNI_TRUE);
      }
...
fillOrDrawRoundRect(Renderer* rdr, jint x, jint y, jint w, jint h,
  ...
  if (stroke && rdr->_dashArray == NULL && aw == ah) {  
    ...
    emitRoundRect(&rdr->_fillerPipeline, x - lineWidth2, y - lineWidth2,
        w + lineWidth, h + lineWidth, aw + lineWidth, ah + lineWidth, 
        XNI_FALSE);
