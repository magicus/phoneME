...
/**
 * Clears touched image memory (0x00000000 transparent). All pixels 
 * touched by painting are reset to mentioned color. This clear function is used 
 * for all surfaces with INT data buffers.
 * @param rdr pointer to Renderer
 * @param height number of rows touched by rendering
 * @see SurfaceTypes, blitClear565(), blitClear8()
 */
void 
blitClearInt(Renderer *rdr, jint height) {
    jint j;
    jint minX, maxX, w;
    jint iidx;
...
/**
 * Clears touched image memory (0x00000000 transparent).
 * @see  blitClearInt(), blitClear8()
 */ 
void 
blitClear565(Renderer *rdr, jint height) {
    jint j;
    jint minX, maxX, w;
    jint iidx;
    ...
    for (j = 0; j < height; j++) {
        ...
        my_clear_mem(shortData+iidx,w * sizeof(jshort));
        imageOffset += imageScanlineStride;
    }
}
...
/**
 * Clears touched image memory (0x00000000 transparent).
 * @see  blitClearInt(), blitClear565()
 */
void
blitClear8(Renderer *rdr, jint height) {
    jint i, j;
    jint minX, maxX, w;
    jint iidx;
    ...
    for (j = 0; j < height; j++) {
        ...
        my_clear_mem(byteData+iidx,w * sizeof(jbyte));
        imageOffset += imageScanlineStride;
    }
}
