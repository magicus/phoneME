...
# include "incls/_GPTableGenerator_thumb2.cpp.incl"
...
void GPTableGenerator::define_long(SourceAssembler::Label &l) {
  if (!GenerateGPTableOnly) {
    ...
    if (GenerateGNUCode) {
      stream()->print_cr("\t.long\t%s", l.symbol());
    } else {
      stream()->print_cr("\tEXTERNDEF %s:PROC", l.symbol());
...
#define INTERP_DUMMY(x) bind(STR(x)); breakpoint();

void GPTableGenerator::generate() {
  if (!GenerateGPTableOnly) {
    Segment seg(this, code_segment, "Code dummies");    
#if !USE_INDIRECT_EXECUTION_SENSOR_UPDATE
    INTERP_DUMMY(indirect_execution_sensor_update)
#endif
    INTERP_DUMMY(compiler_throw_NullPointerException_10)
    INTERP_DUMMY(compiler_throw_ArrayIndexOutOfBoundsException_10)
  }
