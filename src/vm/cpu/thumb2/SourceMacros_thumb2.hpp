...
// Portability macros from ARM -> Thumb2
#define local_addr_at(a, b) locals, __local_addr_at((a), (b))

  int __local_addr_at(int index, int word_offset) {
    return (word_offset * BytesPerWord) +
           (JavaStackDirection * index * BytesPerStackElement);
  }

  void str_local_at(Register value, int index,      Register tmp);
  void str_local_at(Register value, Register index, Register tmp);
  void ldr_local_at(Register value, Register index, Register tmp);
  ...
  enum {
    addr_cpool_index,  // return the address of this cpool entry in "addr"
  ...
  void set_return_type(BasicType type);
  ...
  void pop (Register reg, Condition cond = al,
            StackTypeMode type = full, WritebackMode mode = writeback);
  void push(RegisterSet set, Condition cond = al, 
            StackTypeMode type = full, WritebackMode mode = writeback);
  void pop (RegisterSet set, Condition cond = al, 
            StackTypeMode type = full, WritebackMode mode = writeback);
  void push(int set, Condition cond = al, 
            StackTypeMode type = full, WritebackMode mode = writeback) {
    push((RegisterSet)set, cond, type, mode);
  }
  void pop (int  set, Condition cond = al, 
            StackTypeMode type = full, WritebackMode mode = writeback) {
    pop((RegisterSet)set, cond, type, mode);
  }
  ...
  void push_results(int count,
                     RegisterSet result1=tos,   RegisterSet result2=tmp01, 
                     RegisterSet result3=tmp23, RegisterSet result4=tmp45,
                     RegisterSet result5=emptySet,
                     RegisterSet result6=emptySet);
  ...
  void call_from_interpreter(const char* routine, 
                             bool target_fixes_lr = false,
                             Register tmp = no_reg);
 ...
 protected:
  ...
  void emit_raw(juint instr);

};

#endif /* ENABLE_INTERPRETER_GENERATOR && !defined(PRODUCT) */
