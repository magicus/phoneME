...
jlong JVM::calibrate_hrticks() {
  jlong saved = Universe::current_perf_counts()->hrtick_read_count;
  jlong started = Os::elapsed_counter();
  for (int i=1000; i>0; i--) {
    Os::elapsed_counter();
  }
  jlong end = Os::elapsed_counter();
  Universe::current_perf_counts()->hrtick_read_count = saved;
  return end - started;
}
...
extern "C" void jvm_start_call_vm() {
  Universe::current_perf_counts()->total_in_vm_hrticks 
      -= Os::elapsed_counter();
}
...
static inline jdouble msec_scale(jlong val) {
  return jvm_dmul(jvm_ddiv(jvm_l2d(val),
     jvm_l2d(Universe::current_perf_counts()->hrtick_frequency)),
     1000.0);
}
...
void JVM::print_performance_counters() {
  ...
  if (!(PrintPerformanceCounters        || 
        PrintAllPerformanceCounters     ||
        PrintCompilerPerformanceCounters|| 
        PrintGCPerformanceCounters      || 
        PrintLoadingPerformanceCounters || 
        PrintThreadPerformanceCounters)) {
    return;
  }
...
#else
  {
    ...
    print_performance_counters(pc, hrtick_overhead_per_1000);
...
#if ENABLE_COMPILER
  if (PrintCompilerPerformanceCounters) {
    Compiler::print_detailed_performance_counters();
  }
#endif
}

void JVM::print_performance_counters(JVM_PerformanceCounters *pc, 
                                     jlong hrtick_overhead_per_1000) {
