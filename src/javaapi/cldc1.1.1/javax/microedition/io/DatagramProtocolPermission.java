 ...
 * A DatagramProtocolPermission consists of a URI string but no actions list.
 *
 * The URI string specifies a connection for sending and receiving datagrams. It takes the following form:
 *
 *    datagram://[{host}][:{portrange}]
 *
 * If the {host} string is a DNS name, an asterisk may appear in the leftmost position to indicate 
 * a wildcard match (e.g., "*.sun.com"). The URI string "datagram://*" matches all "client mode" connections; 
 * the string "datagram://" specifies a "server mode" connection.
 *
 * The {portrange} string takes the following form:
 * 
 * portnumber | -portnumber | portnumber-[portnumber]
 *
 * A {portrange} specification of the form "N-" (where N is a port number) signifies all ports numbered 
 * N and above, while a specification of the form "-N" indicates all ports numbered N and below.
 */
...
public final class DatagramProtocolPermission extends GCFPermission {
  ...
  boolean server_mode = false; //just for convinience
  /**
  ...
  public DatagramProtocolPermission(String uri) {
    super(uri);    
    if (!uri.startsWith("datagram://")) {
      throw new IllegalArgumentException("uri shall start with 'datagram://', but is " + uri);
    }
    if (uri.length() == 11/*"datagram://".length()*/) {
      server_mode = true;
      return;
    }
    String full_info = uri.substring(11, uri.length());
   ...
   *
   * p is an instanceof DatagramProtocolPermission, and
   *
   ...
   * p's IP addresses is equal to this object's IP address.
   *
   * If this object is a wildcard domain (such as *.sun.com), and p's canonical name 
   * (the name without any preceding *) ends with this object's canonical host name. 
   * For example, *.sun.com implies *.eng.sun.com..
   *
   * If this object was not initialized with a single IP address, 
   * and one of this object's IP addresses equals one of p's IP addresses.
   *
  ...
  public boolean implies(Permission p) {
    ...
    DatagramProtocolPermission perm = (DatagramProtocolPermission)p;
   ...
   * @return true if obj is a DatagramProtocolPermission and has the same URI string 
   *              as this DatagramProtocolPermission object.
   */
  public boolean equals(Object obj) {
    if (!(obj instanceof DatagramProtocolPermission)) {
      return false;
    }
    DatagramProtocolPermission other = (DatagramProtocolPermission)obj;
   ...
   * is the empty string "", since there are no actions defined for DatagramProtocolPermission.
   ...
   *
   * DatagramProtocolPermission objects must be stored in a manner that allows them to be inserted 
   * into the collection in any order, but that also enables the PermissionCollection implies 
   * method to be implemented in an efficient (and consistent) manner.  
   *
   * @return a new PermissionCollection suitable for storing DatagramProtocolPermission objects.
   */
  public PermissionCollection newPermissionCollection() {
    return new DatagramProtocolPermissionCollection();
  } 
}

/**
 * A DatagramProtocolPermissionCollection stores a collection
 * of DatagramProtocol permissions. DatagramProtocolPermission objects
...
final class DatagramProtocolPermissionCollection extends PermissionCollection
{
    ...
    public DatagramProtocolPermissionCollection() {
    ...
    {
	if (! (permission instanceof DatagramProtocolPermission))
	    throw new IllegalArgumentException("invalid permission: "+
					       permission);
	if (isReadOnly())
	    throw new SecurityException("attempt to add a Permission to a readonly PermissionCollection");

	DatagramProtocolPermission bp = (DatagramProtocolPermission) permission;
	...
	DatagramProtocolPermission bp = (DatagramProtocolPermission) permission;
        ...
        while (search.hasMoreElements()) {
          DatagramProtocolPermission cur_perm = (DatagramProtocolPermission)search.nextElement();
    ...
    public Enumeration elements() {
      return new DPPCEnumeration(client_perms.elements(), server_perms.elements());
    }
}

class DPPCEnumeration implements Enumeration {
  Enumeration en1, en2;

  DPPCEnumeration(Enumeration en1, Enumeration en2) {
