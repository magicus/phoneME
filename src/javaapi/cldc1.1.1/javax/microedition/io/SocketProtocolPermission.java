 ...
 * A SocketProtocolPermission consists of a URI string but no actions list.
 *
 * The URI string specifies a socket stream connection. It takes the following form:
 *
 ...
 * (e.g., "*.sun.com"). The URI string "socket://*" matches all "client mode" connections; 
 * the string "socket://" specifies a server socket connection.
 *
...
public final class SocketProtocolPermission extends GCFPermission {
  ...
  public SocketProtocolPermission(String uri) {
    ...
    if (!uri.startsWith("socket://")) {
      throw new IllegalArgumentException("uri shall start with 'socket://', but is " + uri);
    }
    if (uri.length() == 9/*"socket://".length()*/) {
      server_mode = true;
      return;
    }
    String full_info = uri.substring(9, uri.length());
   ...
   * any of them are not):
   *
   * * p is an instanceof SocketProtocolPermission, and
   *
   ...
   * @return true if obj is a SocketProtocolPermission and has the same URI string 
   *              as this SocketProtocolPermission object.
   */
   ...
   * is the empty string "", since there are no actions defined for SocketProtocolPermission.
   ...
   *
   * SocketProtocolPermission objects must be stored in a manner that allows them to be inserted 
   * into the collection in any order, but that also enables the PermissionCollection implies 
   * method to be implemented in an efficient (and consistent) manner.  
   *
   * @return a new PermissionCollection suitable for storing SocketProtocolPermission objects.
   */
  public PermissionCollection newPermissionCollection() {
    return new SocketProtocolPermissionCollection();
  } 
}

/**
 * A SocketProtocolPermissionCollection stores a collection
 * of SocketProtocol permissions. SocketProtocolPermission objects
...
final class SocketProtocolPermissionCollection extends PermissionCollection
{
    private Vector client_perms;
    private Vector server_perms;

    /**
     * Create an empty SocketProtocolPermissionCollection object.
     *
     */

    public SocketProtocolPermissionCollection() {
    ...
    {
	if (! (permission instanceof SocketProtocolPermission))
	    throw new IllegalArgumentException("invalid permission: "+
					       permission);
	if (isReadOnly())
	    throw new SecurityException("attempt to add a Permission to a readonly PermissionCollection");

	SocketProtocolPermission bp = (SocketProtocolPermission) permission;
	...
	SocketProtocolPermission bp = (SocketProtocolPermission) permission;
        ...
        while (search.hasMoreElements()) {
          SocketProtocolPermission cur_perm = (SocketProtocolPermission)search.nextElement();
    ...
    public Enumeration elements() {
      return new DPPCEnumeration2(client_perms.elements(), server_perms.elements());
    }
}

class DPPCEnumeration2 implements Enumeration {
  Enumeration en1, en2;

  DPPCEnumeration2(Enumeration en1, Enumeration en2) {
