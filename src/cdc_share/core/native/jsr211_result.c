...
static int GET_CH_SIZE(id_size, clas_size) {
	return id_size + 1 + clas_size + 1 + 2 + 1;
}
...
JSR211_RESULT_BUFFER jsr211_create_result_buffer(){
    JSR211_CHAPI_INTERNAL_RESBUF* res = (JSR211_CHAPI_INTERNAL_RESBUF*)
		JSR211_MALLOC(sizeof(JSR211_CHAPI_INTERNAL_RESBUF*));
	if (!res) return 0;
	res->buf = 0;
	res->bufsz = 0;
	res->used = 0;
...
static void fill_ch_buf(const javacall_utf16* id, int id_size, 
    ...
    *buf++ = '\n';
	...
	// put suite_id
	buf += (unsigned short)((unsigned int)suit_id >> 16);
	buf += (unsigned short)((unsigned int)suit_id & 0xFFFF);
...
jsr211_result jsr211_fillHandler(
    ...
    fill_ch_buf(id, id_size, class_name, class_name_size, suit_id, flag, buf);
    return JSR211_OK;
}
...
jsr211_result jsr211_appendHandler(
    ...
    fill_ch_buf(id, id_size, class_name, class_name_size, suit_id, flag, array_->buf + array_->used);
    array_->used += sz;
    array_->buf[0] = n + 1;
    return JSR211_OK;
}
