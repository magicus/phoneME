...
#include <properties_static_data.h>
...
initializeConfig(void) {
    ...
    for (i = 0; NULL != internalKey[i]; i++) {
        javacall_set_property(internalKey[i], internalValue[i], 1,
            JAVACALL_INTERNAL_PROPERTY);
    }
    for (i = 0; NULL != systemKey[i]; i++) {
        javacall_set_property(systemKey[i], systemValue[i], 1,
