...
/**
 * Form to display application info
 */
public class AppInfoForm extends Form implements ActionListener {
	...
	/**
	 * Ctor.  Adds commands, creates icons, initializes font, sets transitions.
	 * Performs generic initialization, not midlet-specific.
	 *
	 * @param mainForm:  main AMS form with the buttons container
	 */
	...
	/**
	 * Sets midlet suite-specific contents to form.
	 *
	...
	public void setContents(int suiteId) throws Throwable {
		...
		displayName = midletSuite.getProperty(
	    ...
	    tmpBuf.setLength(0);
	    tmpBuf.append(Integer.toString((
		...
		ResourceConstants.AMS_SIZE),tmpLabel));
	    ...
	    tmpLabel.setBorderPainted(false);
	    addComponent(createPair(Resource.getString(
		    ResourceConstants.AMS_VERSION),tmpLabel));
	    ...
	    if (midletSuite.isTrusted()) {
		    addComponent(createPair(Resource.getString(
			ResourceConstants.AMS_MGR_AUTH_VENDOR),tmpArea));
	    } else {
		    addComponent(createPair(Resource.getString(
			ResourceConstants.AMS_MGR_VENDOR),tmpArea));
	    }
	    ...
	    addComponent(createPair(Resource.getString(
		ResourceConstants.AMS_MGR_TRUSTED),checkBox));
		...
		addComponent(createPair(Resource.getString(
		    ResourceConstants.AMS_AUTO_START_CONN),tmpLabel));
	    }
	}
	...
	/**
	 * Reads icons for single midlet and midlet suite from storage
	 *
	 */
