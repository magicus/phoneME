 ...
 */
static void fillPendingNotificationObject(jobject destObj,
    ...
    KNI_RESTORE_INT_FIELD(destObj, clazz, "retries",
                          source.retries);
    KNI_RESTORE_PCSL_STRING_FIELD(destObj, clazz, "suiteId",
                             &source.suiteId, temp);
...
Java_com_sun_midp_installer_OtaNotifier_removeDeleteNotification() {
    KNI_StartHandles(1);
    GET_PARAMETER_AS_PCSL_STRING(1, suiteId_str) {
        midpRemoveDeleteNotification(&suiteId_str);
...
Java_com_sun_midp_installer_OtaNotifier_getNumberOfDeleteNotifications() {
 ...
 */
KNIEXPORT KNI_RETURNTYPE_VOID
Java_com_sun_midp_installer_OtaNotifier_fillDeleteNotificationListForRetry() {
    int numberOfObjs;
    int numberOfUrls = 0;
    ...
    do {
        ...
        for (i = 0; i < numberOfObjs; i++) {
            ...
            fillPendingNotificationObject(tempObj, pUrls[i]);
        }
    } while (0);
 ...
 */
KNIEXPORT KNI_RETURNTYPE_VOID
Java_com_sun_midp_installer_OtaNotifier_addInstallNotification() {
    KNI_StartHandles(2);
    GET_PARAMETER_AS_PCSL_STRING(1,suiteId_str)
    GET_PARAMETER_AS_PCSL_STRING(2,url_str) {
        midpAddInstallNotification(&suiteId_str, &url_str);
    } RELEASE_PCSL_STRING_PARAMETER
    RELEASE_PCSL_STRING_PARAMETER
    KNI_EndHandles();
    KNI_ReturnVoid();
 ...
 */
KNIEXPORT KNI_RETURNTYPE_VOID
Java_com_sun_midp_installer_OtaNotifier_removeInstallNotification() {
    KNI_StartHandles(1);
    GET_PARAMETER_AS_PCSL_STRING(1,suiteId_str) {
        midpRemoveInstallNotification(&suiteId_str);
    } RELEASE_PCSL_STRING_PARAMETER
    KNI_EndHandles();
    KNI_ReturnVoid();
}
...
Java_com_sun_midp_installer_OtaNotifier_getInstallNotificationForRetry() {
    int numberOfUrls = 0;
    MidpPendingNotification* pUrls = NULL;
    int i;
    ...
    do {
        ...
        for (i = 0; i < numberOfUrls; i++) {
            if (pcsl_string_equals(&suiteId, &pUrls[i].suiteId)) {
                pUrls[i].retries++;

                fillPendingNotificationObject(destObj, pUrls[i]);
