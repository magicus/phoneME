...
static void initNotification(MidpPendingNotification* pNotification) {
    ...
    pNotification->suiteId = PCSL_STRING_NULL;
...
static void removeNotification(const pcsl_string* filename_str,
    ...
    for (i = 0; i < numberOfElements; i++) {
        if (pcsl_string_equals(&pList[i].suiteId, suiteId_str)) {
            pList[i].retries = -1;
            temp.retries = -1;
            saveNotifications(filename_str, pList, numberOfElements, temp);
            break;
...
static void freeNotificationData(MidpPendingNotification* pNotification) {
    pcsl_string_free(&pNotification->suiteId);
 ...
 * @param url target url for the status message
 */
void midpAddDeleteNotification(const pcsl_string* suiteId_str,
                               const pcsl_string* url_str) {
    pcsl_string filename_str;
    pcsl_string_status rc;
    rc =
      pcsl_string_cat(storage_get_root(),&DELETE_NOTIFY_FILENAME,&filename_str);
    if (PCSL_STRING_OK != rc) {
        return;
    }

    addNotification(&filename_str, suiteId_str, url_str);
 ...
 * @param suiteId suite ID of the notification to remove
 */
void midpRemoveDeleteNotification(const pcsl_string* suiteId) {
    pcsl_string filename;
    pcsl_string_status rc;

    rc = pcsl_string_cat(storage_get_root(), &DELETE_NOTIFY_FILENAME, &filename);
    if (PCSL_STRING_OK != rc) {
        return;
    }
 ...
 * @param url target url for the status message
 */
void midpAddInstallNotification(const pcsl_string* suiteId, const pcsl_string* url) {
    pcsl_string filename;
    pcsl_string_status rc;

    rc = pcsl_string_cat(storage_get_root(), &INSTALL_NOTIFY_FILENAME, &filename );
    if (PCSL_STRING_OK != rc) {
        return;
    }
 ...
 * @param suiteId suite ID of the notification to remove
 */
void midpRemoveInstallNotification(const pcsl_string* suiteId) {
    pcsl_string filename;
    pcsl_string_status rc;
