 ...
 */
static void print_field(char* pszLabel, const pcsl_string* field) {
    printf("%s", pszLabel);
 ...
 * @param field The field to print
 */
static void print_suite_id(char* pszLabel, SuiteIdType suite_id) {
    printf("%s", pszLabel);
    printf("%d", suite_id);
    putchar('\n');
}
...
/**
 * Print a property value to <tt>stdout</tt>.
 *
...
/**
 * List all installed MIDlet suites.
 *
 ...
 */
int listMidlets(int argc, char* argv[]) {
    int   status = -1;
    int   i;
    long  size;
    char* midpHome = NULL;
...
/**
 * List all installed MIDlet suites storage names.
 *
 * @param argc The total number of arguments
 * @param argv An array of 'C' strings containing the arguments
 *
 ...
 *       they should remain as printf calls
 */
int listStorageNames(int argc, char* argv[]) {
	int status = -1;
    int i;
    long size;
    char* midpHome;
