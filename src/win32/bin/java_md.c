...
static char **parse_argv(int *argc, char **argv) {
  ...
  for (argv_i = 0, new_argv_i = 1; argv_i < (*argc - 1); argv_i++, new_argv_i++) {
  ...
  /* a '\\' is found */
      if (argv[argv_i+1][argv_pos] == '\\') {
        consecutive_slashes++;
      } else if ((argv[argv_i+1][argv_pos] == ' ') || (argv[argv_i+1][argv_pos] == '\t')){
  ...
  /* check for consecutive delimiters */
      ...
      new_argv_i++;
  /* we need to malloc more memory because wince didn't calculate argc */
  /* correctly.  So, we malloc the temp_new_argv array, copy the old one */
  /* to the new one, and set new_argv to it. */
      new_argc++;
      ...
      new_argv[new_argv_i] = (char *)malloc(sizeof(char) * new_argv_len);
      new_argv_pos = 0;
    ...
    if (consecutive_slashes) {
      if (consecutive_slashes % 2 == 0) {
        ...
        for (slash_count = 0; slash_count < (consecutive_slashes / 2); \
