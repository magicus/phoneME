...
public final class MediaTunnel {
     ...
     */
    public synchronized void callForegroundEventHandler(SecurityToken token) {
     ...
     * @param token the security token
     */
    public synchronized void callBackgroundEventHandler(SecurityToken token) {
        token.checkIfPermissionAllowed(Permissions.AMS);
        if (map == null) {
            return;
        }
     ...
     * @param token the security token
     */
    public synchronized void callDestroyEventHandler(SecurityToken token) {
        token.checkIfPermissionAllowed(Permissions.AMS);
        if (map == null) {
            return;
        }
