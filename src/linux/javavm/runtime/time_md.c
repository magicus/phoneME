...
static CVMBool _supports_fast_thread_cpu_time = CVM_FALSE;
static CVMBool _is_NPTL = CVM_FALSE;
...
void thread_cpu_clock_init(CVMThreadID *threadID) {
	...
	_supports_fast_thread_cpu_time = CVM_TRUE;
    }
}
...
static CVMInt64
thread_cpu_timeX(CVMThreadID *thread, CVMBool user_sys_cpu_time) {
	...
	if ( fp != NULL ) {
		...
		return ((CVMInt64)sys_time + (CVMInt64)user_time) * (SEC_IN_NANOSECS / clock_tics_per_sec);
	    } else {
		return (CVMInt64)user_time * (SEC_IN_NANOSECS / clock_tics_per_sec);
	    }
	}
    ...
    if (proc_task_unchecked && _is_NPTL) {
    ...
    if (user_sys_cpu_time) {
	return ((jlong)sys_time + (jlong)user_time) * (SEC_IN_NANOSECS / clock_tics_per_sec);
    } else {
	return (jlong)user_time * (SEC_IN_NANOSECS / clock_tics_per_sec);
    }
}
...
static CVMInt64
fast_thread_cpu_time(clockid_t clockid) {
  struct timespec tp;
  int rc = _clock_gettime(clockid, &tp);
  CVMassert(rc == 0);
...
CVMthreadCputime(CVMThreadID *thread) {
    ...
    if (_supports_fast_thread_cpu_time) {
	...
	CVMassert(rc == 0);
	return fast_thread_cpu_time(clockid);
    } else {
	return thread_cpu_timeX(thread, CVM_TRUE /* user + sys */);
    }
}
...
CVMcurrentThreadCputime() {
    if (_supports_fast_thread_cpu_time) {
	return fast_thread_cpu_time(CLOCK_THREAD_CPUTIME_ID);
    } else {
	// return user + sys since the cost is the same
	return thread_cpu_timeX(&(CVMgetEE()->threadInfo), CVM_TRUE /* user + sys */);
    }
}
