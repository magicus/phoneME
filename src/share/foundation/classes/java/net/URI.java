...

	...
	String ns = Normalizer.normalize(s, Normalizer.COMPOSE, 0);
        ...
        try {
            ...
            if (sun.misc.BuildFlags.qAssertsEnabled) assert false;
        }
        return sb.toString();
    }
	...
	StringBuffer sb = new StringBuffer(n);
        byte bb[] = new byte[n];
        char cb[] = new char[n];
	/* we are not handling the error in decoding rightnow.
	...
	for (int i = 0; i < n;) {
		...
		if (sun.misc.BuildFlags.qAssertsEnabled)
                    assert (n - i >= 2);
                bb[j] = decode(s.charAt(++i), s.charAt(++i));
		if (++i >= n)
		    break;
		c = s.charAt(i);
		if (c != '%')
            ...
            try {
                String str = new String(bb, 0, j+1,  "UTF-8");
                cb = str.toCharArray();
                sb.append(str);
            } catch (UnsupportedEncodingException e) {
                if (sun.misc.BuildFlags.qAssertsEnabled) assert false;
