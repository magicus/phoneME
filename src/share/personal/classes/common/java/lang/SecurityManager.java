/*
 * @(#)SecurityManager.java	1.4 06/10/10
 *
...
class SecurityManager {
    ...
    private static Constructor AWTPermissionCtor;
...
*/
    ...
    */
	...
	try {
            if (SecurityConstants.getTopLevelWindowPermission() == null)
                return false;
            
	    checkPermission(SecurityConstants.getTopLevelWindowPermission());
	    return true;
	} catch (SecurityException se) {
	    // just fall through to...
	}
    ...
    public void checkSystemClipboardAccess() {
        if (SecurityConstants.getAccessClipboardPermission() == null) {
           throw new SecurityException("AWT package not available");
        } 
 	checkPermission(SecurityConstants.getAccessClipboardPermission());
    }
    ...
    public void checkAwtEventQueueAccess() {
        if (SecurityConstants.getCheckAwtEventQueuePermission() == null) {
           throw new SecurityException("AWT package not available");
        }
      	checkPermission(SecurityConstants.getCheckAwtEventQueuePermission());
    }
