			    ...
			    CVMBool isRedefine)
{
...
#endif
#ifdef CVM_JVMTI
	    /* Obsolete method bit depends on bit 0 being unused */
	    CVMassert(((CVMUint32)mb & 3) == 0);
#endif
	    cpIdx = get2bytes(context);
     ...
     * If so, we don't add it to the class lists just yet.  Caller of
