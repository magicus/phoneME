/*
 * @(#)JDKMethodInfo.java	1.16 06/10/10
 *
 ...
 *
 * See also ClassClass for VM-specific info associated with each class.
 * See also JDKVM for VM-specific info not associated directly with any class.
 */
import components.*;
...
public class
JDKMethodInfo extends VMMethodInfo implements Const {
    ...
    public JDKMethodInfo( MethodInfo m  ){
	method = m;
    ...
    public int methodOffset( boolean isInterface ){
	...
	for (int pc = 0; pc < code.length; ) {
		...
		rewrite = MethodCallInline(pc, (JDKMethodInfo)targetMethod);
		if (rewrite != null) { 
		    isRewritten = true;
     ...
     */
    private byte[] MethodCallInline(int pc, JDKMethodInfo mb) {
	byte code[] = method.code;
	int opcode = code[pc] & 0xff;
	...
	for (int pc = 0; ; pc++) { 
		...
		if (!((opcode == opc_getfield_quick_w)
		    ...
		    ConstantObject[] cp = mb.parent.constants;

		    FieldConstant fc = (FieldConstant)
			cp[mb.getUnsignedShort(pc+1)];
		    FieldInfo fb = fc.find();
		    char isig = fb.type.string.charAt(0);
		    boolean big = (isig == SIGC_LONG
				   || isig == SIGC_DOUBLE);
		    int offset = fb.instanceOffset;
		    int infoIndex = (opcode == opc_getfield_quick_w ?
				     0 : 1) + (big ? 2 : 0);
		    opPtr = OpcodeInfoSpecial[infoIndex];
		    if (offset <= 255 && stackBase == 0) {
			if (UseLosslessQuickOpcodes) {
			    redoInlining = true;
			} else {
			    opcode2 = (opcode == opc_getfield_quick_w) ?
				(big ? opc_getfield2_quick 
				    : opc_getfield_quick)
				: (big ? opc_putfield2_quick 
				    : opc_putfield_quick);
			    return makeOpcodeResult(false, nextReg, 
						    opPtr.inStack, 3, 
						    opcode2,
						    offset, 0);
			}
	...
	/* ldc_quick         */ new OpcodeInfoType(opc_iadd, 0, 1,
				    OpcodeInfoType.CONSTANT_POOL),
	/* ldc_w_quick       */ new OpcodeInfoType(opc_iadd, 0, 1,
				    OpcodeInfoType.CONSTANT_POOL),
	/* ldc2_w_quick      */ new OpcodeInfoType(opc_iadd, 0, 2,
				    OpcodeInfoType.CONSTANT_POOL),
	...
	/* putfield2_quick   */ new OpcodeInfoType(opc_iadd, 3, 0,
						   OpcodeInfoType.NULL_CHECK),
	/* getstatic_quick   */ new OpcodeInfoType(opc_iadd, 0, 1,
				    OpcodeInfoType.CONSTANT_POOL),
	/* putstatic_quick   */ new OpcodeInfoType(opc_iadd, 1, 0,
				    OpcodeInfoType.CONSTANT_POOL),
	/* getstatic2_quick  */ new OpcodeInfoType(opc_iadd, 0, 2,
				    OpcodeInfoType.CONSTANT_POOL),
	/* putstatic2_quick  */ new OpcodeInfoType(opc_iadd, 2, 0,
				    OpcodeInfoType.CONSTANT_POOL),
	/* invokevirtual_quic*/ new OpcodeInfoType(255),
	/* i.kenonvirtual_qui*/ new OpcodeInfoType(255),
	/* i.kesuper_quick   */ new OpcodeInfoType(255),
	/* invokestatic_quick*/ new OpcodeInfoType(255),
	/* invokeinterface_qu*/ new OpcodeInfoType(255),
	/* i.evirtualobj_quic*/ new OpcodeInfoType(255),
	/* invokeignored_qu*/ new OpcodeInfoType(opc_invokeignored_quick),
	/* new_quick         */ new OpcodeInfoType(opc_iadd, 0, 1,
				    OpcodeInfoType.CAN_ERROR
				    | OpcodeInfoType.CONSTANT_POOL),
	/* anewarray_quick   */ new OpcodeInfoType(opc_iadd, 1, 1,
				    OpcodeInfoType.CAN_ERROR
				    | OpcodeInfoType.CONSTANT_POOL),
	/* multianewarray_qui*/ new OpcodeInfoType(255),
	/* checkcast_quick   */ new OpcodeInfoType(opc_iadd, 1, 1,
				    OpcodeInfoType.CAN_ERROR
				    | OpcodeInfoType.CONSTANT_POOL),
	/* instanceof_quick  */ new OpcodeInfoType(opc_iadd, 1, 1,
				    OpcodeInfoType.CONSTANT_POOL),
	/* invokevirtual_quic*/ new OpcodeInfoType(255),
	/* getfield_quick_w  */ new OpcodeInfoType(opc_getfield_quick_w),
	/* putfield_quick_w  */ new OpcodeInfoType(opc_getfield_quick_w),
	/* nonnull_quick     */ new OpcodeInfoType(opc_nonnull_quick),
    };
