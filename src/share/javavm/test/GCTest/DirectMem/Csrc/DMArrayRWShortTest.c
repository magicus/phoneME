/*
 * @(#)DMArrayRWShortTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayRWShortTest.h"


JNIEXPORT void JNICALL
Java_DMArrayRWShortTest_nSetArray(JNIEnv *env, jobject obj, 
                              jshortArray dstArray, jshortArray srcArray, jint arrayLength)
{
   jint i;
   jshort val;
   jobject lock;
   ...
   CVMD_gcUnsafeExec( ee, {
      CVMArrayOfShort* directSrcArray = (CVMArrayOfShort *)CVMID_icellDirect(ee, srcArray);
      CVMArrayOfShort* directDstArray = (CVMArrayOfShort *)CVMID_icellDirect(ee, dstArray);
      jint rBarrier = 0;
      jint wBarrier = 0;
     ...
     for(i=0; i<arrayLength; i++) {
      CVMD_arrayReadShort(directSrcArray, i, val);
      if(ee->barrier == R_BARRIER_SHORT)
         rBarrier++;
      CVMD_arrayWriteShort(directDstArray, i, val);
      if(ee->barrier == W_BARRIER_SHORT)
         wBarrier++;
     }

      if(rBarrier == arrayLength && wBarrier == arrayLength)
         printf("PASS: DMArrayRWShortTest, Read & Write Barrier Short were invoked\n");
      else
         printf("FAIL: DMArrayRWShortTest, Read & Write Barrier Short were not invoked\n");

      printf("\n");
       
      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayRWShortTest, Gc did not happen in the gc unsafe section\n");
      else
         printf("FAIL: DMArrayRWShortTest, Gc happened in the gc unsafe section\n");
   } );
}
