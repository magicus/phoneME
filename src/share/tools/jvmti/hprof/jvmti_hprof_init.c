...
setup_event_mode(jboolean onload_set_only, jvmtiEventMode state)
{
	...
	if (gdata->cpu_timing) {
	    /*
	     * NOTE: Since CVM has romized system classes, if we
	     * catch these events we end up with the exception 
	     * catcher method accumulating much greater than 100%
	     * of the cost because of the way the hprof code 
	     * inserts the unknown method into the stack trace.
	     */
	    /*	    setEventNotificationMode(state, 
