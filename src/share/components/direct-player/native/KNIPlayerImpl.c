...
static javacall_result doBuffering(KNIDECLARGS KNIPlayerInfo* pKniInfo, long length, javacall_int32 *returnValue) {
    void *nBuffer;
    long nBufferSize;
...
LockAudioMutex();            
    if (pKniInfo && pKniInfo->pNativeHandle) {
        JAVACALL_MM_ASYNC_EXEC(
            ret,
            doBuffering(KNIPASSARGS pKniInfo, length, &returnValue),
