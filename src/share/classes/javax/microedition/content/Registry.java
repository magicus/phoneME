...
import com.sun.j2me.content.RegistryFactory;
...
public class Registry {
    ...
    public static ContentHandlerServer getServer(String classname)
        ...
        synchronized (factory) {
            try {
                RegistryImpl impl = factory.getRegistryImpl(classname);
                ContentHandler server = impl.getServer();

                if (server != null) {
                    if (!(server instanceof ContentHandlerServerImpl)) {
                        server = new ContentHandlerServerImpl(impl, server);
                    }
    ...
    /**
     * Constructor for initialized Registry.
     * @param impl internal implementation of creating Registry.
     */
    ...
    {
        /* just to keep logic consistency - move body into RegistryImpl
     ...
     */
        InvocationImpl invocImpl = invocation.getInvocImpl();
        InvocationImpl prevImpl = previous == null? null:
                                                    previous.getInvocImpl();
    ...
    public Invocation getResponse(boolean wait) {
        InvocationImpl responseImpl = impl.getResponse(wait);
        if (responseImpl != null) {
            return new Invocation(responseImpl);
        }
