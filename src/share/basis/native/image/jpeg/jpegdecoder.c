...
sun_jpeg_skip_input_data(j_decompress_ptr cinfo, long num_bytes)
{
    ...
    while (num_bytes > 0) {
	ret = (*env)->CallIntMethod(env, src->hInputStream,
				    CachedIDs.java_io_InputStream_readMID,
				    src->hInputBuffer, 0, buflen);
					      ...
					      jbyteArray hInputBuffer)
{
  ...
  if (hInputBuffer == 0 || hInputStream == 0) {
    (*env)->ThrowNew (env, CachedIDs.java_lang_NullPointerExceptionClass, NULL);
    return;
  }
  ...
  if (setjmp(jerr.setjmp_buffer)) {
	...
	(*cinfo.err->format_message) ((struct jpeg_common_struct *) &cinfo,
				      buffer);
	(*env)->ThrowNew (env, CachedIDs.sun_awt_image_ImageFormatExceptionClass, buffer);
    }
    return;
  }
   ...
   */
  RELEASE_ARRAYS(env, &jsrc);
  ret = (*env)->CallBooleanMethod(env, this, CachedIDs.JPEGImageDecoder_sendHeaderInfoMID,
  ...
  do {
	  ...
	  if (grayscale) {
	      RELEASE_ARRAYS(env, &jsrc);
	      ret = (*env)->CallBooleanMethod(env, this, CachedIDs.JPEGImageDecoder_sendPixelsByteMID,
					      jsrc.hOutputBuffer,
		  ...
		  *--ip = pixel;
	      ...
	      RELEASE_ARRAYS(env, &jsrc);
	      ret = (*env)->CallBooleanMethod(env, this, CachedIDs.JPEGImageDecoder_sendPixelsIntMID,
					      jsrc.hOutputBuffer,
					      cinfo.output_scanline - 1);
	  }
	  if ((*env)->ExceptionOccurred(env) || !ret ||
