...
public class ConnectionReceiver implements Runnable {
   ...
   static int APPMGR_PORTNUM = initAppManagerPort(); 
   static ServerSocket serverSocket;
   private static boolean debug = false; // Enable/disable debug output

   private static int initAppManagerPort() {
      // If "cdcams.ixcport" property is not set, the default is 35218.
      return ((Integer)AccessController.doPrivileged(
                 new GetIntegerAction("cdcams.ixcport", 35218))).intValue();
   }
   ...
   public void run() {
      ...
      while (true) {
        ...
        if (clientSocket != null) {
           ...
           Worker.execute(Thread.NORM_PRIORITY, executor);
        }
      }
    }

    static void startAppManagerSocket() {
       if (serverSocket == null) {
    ...
    class RemoteMethodExecutor implements Runnable {
       ...
       public void run() {
          ...
          boolean isCDCAmsIxcRegistry = false;
          ...
          try {
             ...
             if (exportedObject != null) {
                ...
                isCDCAmsIxcRegistry = 
                   (remoteObject instanceof CDCAmsIxcRegistry);
             }

             if (debug) debugOut("Found corresponding Object " + remoteObject + ", isCDCAmsIxcRegistry=" + isCDCAmsIxcRegistry);
                ...
                try {
                   if (paramTypes.length > 0) {
                      ...
                      IxcInputStream oin = new IxcInputStream(in, 
                               exportedObject.context, isCDCAmsIxcRegistry);
