...
class DerIndefLenConverter {
    ...
    private void parseTag() throws IOException {
        ...
        if (isEOC(data[dataPos]) && (data[dataPos + 1] == 0)) {
            ...
            for (index = ndefsList.size()-1; index >= 0; index--) {
                ...
                // have a matching EOC
                elem = ndefsList.elementAt(index);
            ...
            ndefsList.setElementAt(sectionLenBytes, index);
    ...
    private int parseLength() throws IOException {
        ...
        if (isIndefinite(lenByte)) {
            ndefsList.addElement(new Integer(dataPos));
            return curLen;
        }
    ...
    private void writeLengthAndValue() throws IOException {
        ...
        int lenByte = data[dataPos++] & 0xff;
        if (isIndefinite(lenByte)) {
            byte[] lenBytes = (byte[])ndefsList.elementAt(index++);
