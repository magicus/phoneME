...
final public class ObjectIdentifier {
     ...
     *
     * @param values array of integers used to construct object identifier
     * @exception IOException if this is not a legal OID
     */
    public ObjectIdentifier (int values []) throws IOException {
        int len = values.length;
        checkValidOid(values, len);
        components = new int[len];
        System.arraycopy(values, 0, components, 0, len);
    ...
    private static void putComponent (DerOutputStream out, int val)
            throws IOException {
        int     i;
        // IMPL_NOTE: val must be <128*128*128*128 here, otherwise, 4 bytes
        // is not enough to hold it. Will address this later.
        byte    buf [] = new byte [4] ;
    ...
    /**
     * Calculates a hash code of this object identifier.
     *
     * @return hash code of this object identifier
     */
    public int hashCode() {
        int h = componentLen;
        for (int i = 0; i < componentLen; i++) {
