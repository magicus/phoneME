 ...
 * @file
 *
 * This file implements all the necessary interfaces for SoS client.
...
static void pid_dec(void);
static void pid_inc(void);
...
static void remote_resync(void);
...
javacall_result javacall_socket_open_start(unsigned char *ipBytes, int port,
    ...
    sprintf(buff,"javacall_socket_open_start() >> port=%d , "
                 "ipBytes: %d.%d.%d.%d\n",
 ...
 * See javacall_socket.h for definition.
 */
javacall_result javacall_socket_open_finish(javacall_handle handle,
                                            void *context) {   
    return JAVACALL_FAIL;
}
...
javacall_result javacall_socket_read_start(javacall_handle handle,
    ...
    if (len > maxlen)
        len = maxlen;
    bytesRead = remote_recv((int)handle,(char *)pData, len);
    if (bytesRead == 0) {
        pData = NULL;
        *pBytesRead = 0;
        /*
...
javacall_result javacall_socket_write_start(javacall_handle handle,
    ...
    /* Since we can't send any packet larger than MTU,
     * cut the length to fit it */
    maxlen = MTU - HEADER_LENGTH - CRC_LENGTH;
    if (len > maxlen)
        len = maxlen;

    bytesWritten = remote_send((int)handle, pData, len);
 ...
 * See javacall_socket.h for definition.
 */ 
javacall_result javacall_socket_close_start(javacall_handle handle,
                                            void **pContext) {
    int status;
...
javacall_result javacall_network_gethostbyname_start(char *hostname,
    ...
    if (javacall_network_init_start() != JAVACALL_OK) {
        REPORT("ERROR: javacall_network_gethostbyname_start: "
               "init failed, retuning error\n");   
        return JAVACALL_FAIL;
    }
 ...
 * See javacall_network.h for definition.
 *
 * Since the start function never returns WOULDBLOCK, this
 ...
 * See javacall_socket.h for definition.
 */ 
javacall_result javacall_socket_getlocaladdr(javacall_handle handle,
                                             char *pAddress) {
    return JAVACALL_FAIL;
}
 ...
 * See javacall_socket.h for definition.
 */ 
javacall_result javacall_socket_getremoteaddr(javacall_handle handle,
                                              char *pAddress) {
    return JAVACALL_FAIL;
}
 ...
 * See javacall_socket.h for definition.
 */ 
javacall_result javacall_socket_getlocalport(javacall_handle handle,
                                             int *pPortNumber) {
    return JAVACALL_FAIL;
}
 ...
 * See javacall_socket.h for definition.
 */ 
javacall_result javacall_socket_getremoteport(javacall_handle handle,
                                              int *pPortNumber) {
    return JAVACALL_FAIL;
}
 ...
 * See javacall_network.h for definition.
 */ 
int javacall_network_error(javacall_handle handle) {
    return 0;
}
...
javacall_result javacall_network_finalize_start(void) {
    if (netinit) {
        /*
         * need revisit - after disconnecting from sos server,
         * "crc error" occurs when re-connecting
         */
        /*
 ...
 *****************************************************/
static int invoke_remote(int func, unsigned char handle,
...
resend:
    ...
    case FUNC_OPEN:
        ...
        if (send_packet(maketype(TYPE_INVOKE, FUNC_OPEN),
                        get_cid(packet), pid, handle, (char *)in, in_len) < 0) {
            return JAVACALL_FAIL;
        }
        break;
    ...
    case FUNC_SEND:
        if ((out == NULL) || (out_len != 4))
            return JAVACALL_INVALID_ARGUMENT;
        if ((status = send_packet(maketype(TYPE_INVOKE, FUNC_SEND),
                                  get_cid(packet), pid, handle,
                                  (char *)in, in_len)) != JAVACALL_OK) {
            return status;
        }
        break;
    ...
    case FUNC_RECV:
        {
            ...
            if ((status = send_packet(maketype(TYPE_INVOKE, FUNC_RECV),
                                      get_cid(packet), pid,
                                      handle,(char*)tmp, 2)) != JAVACALL_OK) {
                return status;
            }
        }
        break;
    ...
    case FUNC_SHUTDOWN:
        if (send_packet(maketype(TYPE_INVOKE, FUNC_SHUTDOWN),
                        get_cid(packet), pid, handle, (char *)in, in_len) < 0) {
            return JAVACALL_FAIL;
        }
        break;
    ...
    case FUNC_RESYNC:
        if (send_packet(maketype(TYPE_INVOKE, FUNC_RESYNC),
                        get_cid(packet), 0/*pid*/, handle, NULL, 0) < 0) {
    ...
    case FUNC_GETIPNUMBER:
        ...
        if (send_packet(maketype(TYPE_INVOKE, FUNC_GETIPNUMBER),
                        get_cid(packet), pid, handle, (char *)in, in_len) < 0) {
    ...
    case FUNC_CLOSE:
        if (send_packet(maketype(TYPE_INVOKE, FUNC_CLOSE),
                        get_cid(packet), pid, handle,  NULL, 0) < 0) {
            return JAVACALL_FAIL;
        }
        break;

    case FUNC_PUTSTRING:
        if (send_packet(maketype(TYPE_INVOKE, FUNC_PUTSTRING),
                        get_cid(packet), pid, handle, (char *)in, in_len) < 0) {
            return JAVACALL_FAIL;
        }
        break;
    ...
    if (get_klass(packet) == TYPE_CHALLENGE) {
        /* If receive CHALLENGE, client need re-send the request */
        goto resend;
    }
    ...
    if ((get_klass(packet) == TYPE_RESULT) && (pid == get_pid(packet))) {
        ...
        /* CLASS & PID are both match for the expected answer */
        char* p;

        /* According to the protocol, since we received an answer
...
int serial_DebugPutString(const char* buf) {
    ...
    if (invoke_remote(FUNC_PUTSTRING, PROTOCOL_VERSION,
                      (void *)buf, len>maxlen?maxlen:len, NULL, 0) == 0) {
        return 0;
    } else {
        return -1;
    }
...
static int remote_getipnumber(char* host) {
    ...
    if (invoke_remote(FUNC_GETIPNUMBER, PROTOCOL_VERSION,
                      host, strlen((char *)host), &ipn, 4) == JAVACALL_OK) {
        return ipn;
    } else {
        return JAVACALL_FAIL;
    }
...
static int remote_open(int ipn, int port) {
    ...
    if (invoke_remote(FUNC_OPEN, PROTOCOL_VERSION,
                      par, 6, &result, 4) == JAVACALL_OK) {
        return result;
    } else {
        return JAVACALL_FAIL;
    }
...
/**
 * FUNC: int send_packet(int func, unsigned char cid,
 *                       unsigned char pid, char* content, int clen)
 ...
 * NOTE:
 */
static int send_packet(int func, unsigned char cid, unsigned char pid,
                       unsigned char handle, char* content, int clen) {
...
static unsigned short int calc_crc(unsigned short crc, unsigned char* buf,
                                   int len) {
    unsigned char *p, *lim;
