 ...
 * @param suite_id string identifying the suite or bundle where content handler is located
 ...
 * Function should be called sequentially untill JAVACALL_CHAPI_ERROR_NO_MORE_ELEMENTS is returned
 ...
 *                (pos_id value is arbitrary number and not is index or position in enumeraion
 ...
 *         JAVACALL_CHAPI_ERROR_BUFFER_TOO_SMALL if bufer too small to keep result
 ...
 *         error code if failure occurs
 */
javacall_result javacall_chapi_enum_handlers_by_suite_id(int suite_id, int* pos_id, /*OUT*/ javacall_utf16*  handler_id_out, int* length)
{
    return JAVACALL_NOT_IMPLEMENTED;
}
...
/**
 * Enumerate registered content handlers for which id is equal or begins with handler_id_prefix
 * Search is case-sensitive
 * Function should be called sequentially untill JAVACALL_CHAPI_ERROR_NO_MORE_ELEMENTS is returned
 * Returned between calls values are not guaranteed to be unique, it is up to caller to extract unique values if required
 *
 * @handler_id_prefix searched handler's id prefix
 * @param pos_id  pointer to integer that keeps postion's information in enumeration
 *                before first call integer pointed by pos_id must be initialized to zero, 
 *                each next call it should have value returned in previous call
 *                (pos_id value is arbitrary number and not is index or position in enumeraion
 ...
 *         error code if failure occurs
 */
javacall_result javacall_chapi_enum_handlers_by_id_prefix(javacall_const_utf16_string handler_id_prefix, int* pos_id, /*OUT*/ javacall_utf16*  handler_id_out, int* length)
{
    return JAVACALL_NOT_IMPLEMENTED;
}
 ...
 * @return JAVACALL_OK if operation was successful, 
 *         JAVACALL_CHAPI_ERROR_BUFFER_TOO_SMALL if classname_out bufer lenght is too small to keep result
 *         error code if failure occurs
 */
