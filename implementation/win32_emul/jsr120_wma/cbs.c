 ...
 *
 * @return <tt>JAVACALL_OK</tt> if CBS service is available
...
javacall_result javacall_cbs_add_listening_msgID(unsigned short msgID) {
    ...
    if (free == -1) {
	 javautil_debug_print (JAVACALL_LOG_ERROR, "cbs", "ports amount exceeded");
        return JAVACALL_FAIL;
    }
 ...
 * the specified ID should not be delivered to the WMA implementation.
 *
