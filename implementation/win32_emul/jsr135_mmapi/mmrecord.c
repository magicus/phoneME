...
/**
 * Recoder player handle
 */
typedef struct {
    int             isolateId;
    int             playerId;               /* Unique player ID that is generated by Java */
...
static javacall_result create_wav_file(const char* name, WAVEFORMATEX* pFormat, /*OUT*/ HMMIO* hFile);
static javacall_result write_wav_file(recorder_handle* hRecord, const char* buffer, long size);
static javacall_result add_input_buffer(recorder_handle* hRecord, WAVEFORMATEX* pFormat);
...
static void CALLBACK waveInProc(HWAVEIN hwi, UINT uMsg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2)
{
    ...
    JC_MM_DEBUG_PRINT1("[record] waveInProc %d %d\n", hRecord->isRecording);

    if (uMsg != WIM_DATA)
        return;
    
    if (hRecord->hWAVEIN && hRecord->pHdr) {
        mmReturn = waveInUnprepareHeader(hRecord->hWAVEIN, hRecord->pHdr, sizeof(WAVEHDR));
    ...
    if (hRecord->isRecording) {
        ...
        if (MMSYSERR_NOERROR != mmReturn) {
            JC_MM_DEBUG_PRINT("[record] Can't reuse input buffer\n");
        }
...
static javacall_result create_wav_file(const char* name, WAVEFORMATEX* pFormat, /*OUT*/ HMMIO* hFile)
{
    *hFile = mmioOpen((LPSTR)name, NULL, MMIO_CREATE|MMIO_WRITE|MMIO_EXCLUSIVE|MMIO_ALLOCBUF);
    if (NULL != *hFile) {
...
static javacall_result write_wav_file(recorder_handle* hRecord, const char* buffer, long size)
{
    if (hRecord && hRecord->hFile && buffer && size > 0) {
        ...
        JC_MM_DEBUG_PRINT2("[record] write to wav file %d %d\n", size, ret);
        return JAVACALL_OK;
    }
...
static javacall_result init_recording(recorder_handle* hRecord)
{
    ...
    if (NULL == hRecord->hWAVEIN) {
        ...
        mmReturn = waveInOpen(&hRecord->hWAVEIN, WAVE_MAPPER, &format, (DWORD)waveInProc, 
                              (DWORD)hRecord, CALLBACK_FUNCTION);      
 ...
 * Not Implemented Yet!
 */
static javacall_result recorder_set_recordsize_limit(javacall_handle handle, /*INOUT*/ long* size)
{
    return JAVACALL_FAIL;
}
 ...
 */
static javacall_result recorder_get_recorded_data(javacall_handle handle, /*OUT*/ char* buffer, 
