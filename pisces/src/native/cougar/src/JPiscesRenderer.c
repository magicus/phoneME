...
Java_com_sun_pisces_PiscesRenderer_initialize() {
  ...
  jint imageType = KNI_GetParameterAsInt(2);
  ...
  if (initializeRendererFieldIds(objectHandle)) {
    ...
    rdr = renderer_create(surface, imageType);
...
KNIEXPORT KNI_RETURNTYPE_VOID 
Java_com_sun_pisces_PiscesRenderer_finalize() {
  KNI_StartHandles(1);
  KNI_DeclareHandle(objectHandle);
...
Java_com_sun_pisces_PiscesRenderer_setStroke__IIII_3II() {
  ...
  if (!KNI_IsNullHandle(arrayHandle)) {
    ...
    dashArray = (jint*)pcsl_mem_malloc(length * sizeof(jint));
...
Java_com_sun_pisces_PiscesRenderer_setTextureImpl() {
  ...
  data = (jint*)pcsl_mem_malloc((width + 2) * (height + 2) * sizeof(jint));
  ...
  if (NULL == data) {
    ...
    renderer_setTexture(rdr, data, width, height, repeat, &textureTransform);
