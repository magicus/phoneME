...
static void blend888(jint *intData, jint aval, 
    jint cred, jint cgreen, jint cblue);
static void blend8888(jint *intData, jint aval, 
    jint sred, jint sgreen, jint sblue);
static void blend565(jshort *shortData, jint pix);
static void blend8(jbyte *byteData, jint aval, jint cgray);
static void blendLine(void *data, jint imageType, jint offset, jint stride,
...
fillRectSrcOver(Renderer* rdr,
  ...
  jint RECT_CORNER_ALPHA_SHIFT  = ALPHA_SHIFT -
    (rdr->_SUBPIXEL_LG_POSITIONS_X + rdr->_SUBPIXEL_LG_POSITIONS_Y);
  ...
  if (width > 0 && height > 0) {
    offset = imageOffset + iy0*imageScanlineStride + ix0*imagePixelStride;
...
/*              if(scrOrient == com_sun_agui_ril_AGUISurfaceInfo_ORIENTATION_X_RIGHT_Y_DOWN)
                { */
                        for (j = height; j > 0; j--) {
                                int cnt = width;
                                int sdoff = 0;
...
/*              if(scrOrient == com_sun_agui_ril_AGUISurfaceInfo_ORIENTATION_Y_UP_X_RIGHT)
                {
                        for (j = width; j > 0; j--) {
                          int cnt = height;
                          int sdoff = 0;
                          ...
                          {
                                  *(unsigned short *)(sd+sdoff) = shortVal;
                                  sdoff += imageScanlineStride;
                                  cnt--;
                          }
                          while(cnt)
                          {
                                  *(unsigned int *)(sd+sdoff) = cval;
                                  sdoff += 2*imageScanlineStride;
                                  cnt -= 2;
                          }
                          sd += imagePixelStride;
                        }
  ...
  case TYPE_INT_RGB:
    if (alphaUL > 0) {
      blend888(&intData[imageOffset + (iy0 - 1)*imageScanlineStride +
    ...
    if (alphaLL > 0) {
      blend888(&intData[imageOffset + (iy1 + 1)*imageScanlineStride +
                        (ix0 - 1)*imagePixelStride],
  ...
  case TYPE_INT_ARGB:
    if (alphaUL > 0) {
      blend8888(&intData[imageOffset + (iy0 - 1)*imageScanlineStride +
                        (ix0 - 1)*imagePixelStride],
               alphaUL, cred, cgreen, cblue);
    }
    if (alphaUR > 0) {
    ...
    if (alphaLL > 0) {
      blend8888(&intData[imageOffset + (iy1 + 1)*imageScanlineStride +
                        (ix0 - 1)*imagePixelStride],
               alphaLL, cred, cgreen, cblue);
    }
    if (alphaLR > 0) {
  ...
  case TYPE_USHORT_565_RGB:
    if (alphaUL > 0) {
      blend565(&shortData[imageOffset + (iy0 - 1)*imageScanlineStride +
                          (ix0 - 1)*imagePixelStride],
               (alphaUL<<24) | intVal);
    }
    if (alphaUR > 0) {
    ...
    if (alphaLL > 0) {
      blend565(&shortData[imageOffset + (iy1 + 1)*imageScanlineStride +
                          (ix0 - 1)*imagePixelStride],
               (alphaLL<<24) | intVal);
    }
    if (alphaLR > 0) {
  ...
  case TYPE_BYTE_GRAY:
    if (alphaUL > 0) {
      blend8(&byteData[imageOffset + (iy0 - 1)*imageScanlineStride + 
                      (ix0 - 1)*imagePixelStride],
             alphaUL, cred);
    }
    if (alphaUR > 0) {
    ...
    if (alphaLL > 0) {
      blend8(&byteData[imageOffset + (iy1 + 1)*imageScanlineStride +
                       (ix0 - 1)*imagePixelStride],
             alphaLL, cred);
    }
    if (alphaLR > 0) {
  ...
  for (j = 0; j < height; j++) {
    ...
    while (a < am) {
      ...
      if (aval == MAX_ALPHA) {
        ...
        blend565(&shortData[iidx], (aval << 24) | (cval & 0xffff));
      }
      iidx += imagePixelStride;
    }
...
genRadialGradientPaint(Renderer *rdr, jint height)
{
    ...
    a00 = rdr->_gradient_inverse_transform.m00/65536.0;
    a01 = rdr->_gradient_inverse_transform.m01/65536.0;
    a02 = rdr->_gradient_inverse_transform.m02/65536.0;
    a10 = rdr->_gradient_inverse_transform.m10/65536.0;
    a11 = rdr->_gradient_inverse_transform.m11/65536.0;
    a12 = rdr->_gradient_inverse_transform.m12/65536.0;

    cx = rdr->_rg_cx/65536.0;
    cy = rdr->_rg_cy/65536.0;
    fx = rdr->_rg_fx/65536.0;
    fy = rdr->_rg_fy/65536.0;
    r = rdr->_rg_radius/65536.0;
    rsq = r*r;
  ...
  for (j = 0; j < height; j++) {
    ...
    while (a < am) {
      ...
      if (aval == MAX_ALPHA) {
        ...
        blend888(&intData[iidx], aval, (cval >> 16) & 0xff, 
  ...
  for (j = 0; j < height; j++) {
    ...
    while (a < am) {
        ...
        if (aval == MAX_ALPHA) {
          ...
          blend8888(&intData[iidx], aval, (cval >> 16) & 0xff, 
            (cval >> 8) & 0xff, cval & 0xff);
        }
  ...
  for (j = 0; j < height; j++) {
    ...
    while (a < am) {
        ...
        if (aval == MAX_ALPHA) {
            ...
            blend8(&byteData[iidx], aval, cval & 0xff);
        }
...
clearRect8888(Renderer *rdr, jint x, jint y, jint w, jint h) {
  jint cval = (rdr->_calpha << 24) | (rdr->_cred << 16) | (rdr->_cgreen << 8) | 
      rdr->_cblue;
...
static void 
blend565(jshort *shortData, jint pix) {
/* assume cred, cgreen, cblue are at the correct bit depths */
  ...
  *shortData = ((dwAlphaRBtemp + (((coRB - dwAlphaRBtemp) * dw6bitOpacity) >> 6)) & 0xf81f) |
          ((dwAlphaGtemp + (((coG - dwAlphaGtemp) * dw6bitOpacity) >> 6)) & 0x07e0);
}

static void 
blend8(jbyte *byteData,
    jint aval,
    jint cgray) {
  jint gray;
  jint ngray;
...
blendLine(void *data, jint imageType, jint offset, jint stride,
  ...
  if (imageType == TYPE_INT_RGB) {
    intData = (jint *)data;
    for (i = 0; i < length; i++) {
      blend888(&intData[offset], alpha, red, green, blue);
      offset += stride;
    }
  } else if (imageType == TYPE_INT_ARGB) {
    intData = (jint *)data;
    for (i = 0; i < length; i++) {
      blend8888(&intData[offset], alpha, red, green, blue);
      offset += stride;
    }
  } else if (imageType == TYPE_USHORT_565_RGB) {
    ...
    for (i = 0; i < length; i++) {
      blend565(&shortData[offset], cval);
      offset += stride;
    }
  } else if (imageType == TYPE_BYTE_GRAY) {
    byteData = (jbyte *)data;
    for (i = 0; i < length; i++) {
      blend8(&byteData[offset], alpha, red);
      offset += stride;
    }
  }
}
