...
piscesmath_moduleInitialize() {
  if (sintab == NULL) {
    jint i;
    sintab = (jint*)PISCESmalloc((PISCES_SINTAB_ENTRIES + 1) * sizeof(jint));
    if (sintab == NULL) {
      return XNI_FALSE;
    }
...
piscesmath_isqrt(jint x) {
  ...
  do {
    ...
    remHi = (remHi << 2) | ((unsigned int)remLo >> 30); // N.B. - unsigned shift R
    remLo <<= 2;
    root <<= 1;
    testdiv = (root << 1) + 1;
    if (remHi >= testdiv) {
...
piscesmath_lsqrt(jlong x) {
  ...
  do {
    ...
    remHi = (remHi << 2) | ((ulong64)remLo >> 62); // N.B. - unsigned shift R
    remLo <<= 2;
    root <<= 1;
    testdiv = (root << 1) + 1;
    if (remHi >= testdiv) {
