...
static void stroker_setParameters(Pipeline* pipeline, jint lineWidth,
    jint capStyle, jint joinStyle, jint miterLimit, Transform6* transform);
...
static void dasher_setParameters(Pipeline* pipeline, jint* dash, 
    jint dashLength, jint phase, Transform6* transform);
...
static jboolean flattener_flatEnough_4(Pipeline* pipeline, 
    jint x0, jint y0, jint x1, jint y1, jint x2, jint y2, jint x3, jint y3);
...
static void stroker_computeOffset(Pipeline* pipeline, 
    jint x0, jint y0, jint x1, jint y1, jint* m);
...
static jint stroker_computeRoundJoin(Pipeline* pipeline, 
    jint cx, jint cy, jint xa, jint ya, jint xb, jint yb, jint side, 
...
static void stroker_drawMiter(Pipeline* pipeline, jint px0, jint py0,
    jint x0, jint y0, jint x1, jint y1, jint omx, jint omy, jint mx, jint my,
    jboolean rev);
...
pipeline_initializeStroker(Pipeline* pipeline, void* param) {
  ...
  pipeline->stroker_penIncluded = (jboolean*)PISCESmalloc(16 * sizeof(jboolean));
...
pipeline_setBBoxParameters(Pipeline* pipeline,
    ...
    jint m00, jint m01, jint m10, jint m11, jint m02, jint m12) {
  Transform6 biasedTransform;
  ...
  transformer_setTransform(pipeline, &biasedTransform);
  stroker_setParameters(pipeline, lineWidth, CAP_SQUARE, JOIN_MITER, miterLimit,
      &biasedTransform);
}
...
transformer_moveTo(Pipeline* pipeline, jint x0, jint y0) {
  ...
  if (pipeline->transformer_scaleAndTranslate) {
    ...
    tx0 = pipeline->transformer_m00*x0 + pipeline->transformer_m01*y0 + pipeline->transformer_m02;
    ty0 = pipeline->transformer_m10*x0 + pipeline->transformer_m11*y0 + pipeline->transformer_m12;
  }
...
transformer_lineTo(Pipeline* pipeline, jint x1, jint y1) {
  ...
  if (pipeline->transformer_scaleAndTranslate) {
    ...
    tx1 = pipeline->transformer_m00*x1 + pipeline->transformer_m01*y1 + pipeline->transformer_m02;
    ty1 = pipeline->transformer_m10*x1 + pipeline->transformer_m11*y1 + pipeline->transformer_m12;
  }
...
transformer_quadTo(Pipeline* pipeline, jint x1, jint y1, 
  ...
  if (pipeline->transformer_scaleAndTranslate) {
    ...
    tx1 = pipeline->transformer_m00*x1 + pipeline->transformer_m01*y1 + pipeline->transformer_m02;
    ty1 = pipeline->transformer_m10*x1 + pipeline->transformer_m11*y1 + pipeline->transformer_m12;
    tx2 = pipeline->transformer_m00*x2 + pipeline->transformer_m01*y2 + pipeline->transformer_m02;
    ty2 = pipeline->transformer_m10*x2 + pipeline->transformer_m11*y2 + pipeline->transformer_m12;
  }
...
transformer_cubicTo(Pipeline* pipeline, jint x1, jint y1, 
  ...
  if (pipeline->transformer_scaleAndTranslate) {
    ...
    tx2 = pipeline->transformer_m00*x2 + pipeline->transformer_m01*y2 + pipeline->transformer_m02;
    ty2 = pipeline->transformer_m10*x2 + pipeline->transformer_m11*y2 + pipeline->transformer_m12;
    tx3 = pipeline->transformer_m00*x3 + pipeline->transformer_m01*y3 + pipeline->transformer_m02;
    ty3 = pipeline->transformer_m10*x3 + pipeline->transformer_m11*y3 + pipeline->transformer_m12;
  }
...
stroker_moveTo(Pipeline* pipeline, jint x0, jint y0) {
  if (pipeline->stroker_lineToOrigin) {
    ...
    stroker_lineToImpl(pipeline, pipeline->stroker_sx0, pipeline->stroker_sy0, 
...
stroker_lineToImpl(Pipeline* pipeline, jint x1, jint y1, jboolean joinSegment) {
  ...
  if (!pipeline->stroker_started) {
    ...
    jboolean ccw = stroker_isCCW(pipeline->stroker_px0, pipeline->stroker_py0, 
        x0, y0, x1, y1);
    if (joinSegment) {
      if (pipeline->stroker_joinStyle == JOIN_MITER) {
        stroker_drawMiter(pipeline, pipeline->stroker_px0, pipeline->stroker_py0, 
            x0, y0, x1, y1, pipeline->stroker_omx, pipeline->stroker_omy, 
            mx, my, ccw);
        ...
        stroker_drawRoundJoin(pipeline, x0, y0, 
            ...
            mx, my, 0, XNI_FALSE, ccw, ROUND_JOIN_THRESHOLD);
      }
    } else {
      // Draw internal joins as round
      stroker_drawRoundJoin(pipeline, x0, y0, 
          pipeline->stroker_omx, pipeline->stroker_omy,
          mx, my, 0, XNI_FALSE, ccw, ROUND_JOIN_INTERNAL_THRESHOLD);
    } 
...
stroker_close(Pipeline* pipeline) {
  ...
  if (pipeline->stroker_joinSegment) {
    if (joinStyle == JOIN_MITER) {
      stroker_drawMiter(pipeline, pipeline->stroker_px0, pipeline->stroker_py0, 
          x0, y0, sx0, sy0, pipeline->stroker_omx, pipeline->stroker_omy, 
          mx, my, ccw);
  ...
  ccw = stroker_isCCW(x0, y0, sx0, sy0, pipeline->stroker_sx1, pipeline->stroker_sy1);
  ...
  if (!ccw) {
    if (joinStyle == JOIN_MITER) {
      ...
      stroker_drawRoundJoin(pipeline, sx0, sy0, mx, my, 
          mx0, my0, 0, XNI_FALSE, XNI_FALSE, ROUND_JOIN_THRESHOLD);
    }
  }

  pipeline->stroker_next_lineTo(pipeline, sx0 + mx0, sy0 + my0);
  pipeline->stroker_next_lineTo(pipeline, sx0 - mx0, sy0 - my0);  // same as reverse[0], reverse[1]
  ...
  if (ccw) {
    if (joinStyle == JOIN_MITER) {
      ...
      stroker_drawRoundJoin(pipeline, sx0, sy0, -mx, -my, 
          -mx0, -my0, 0, XNI_TRUE, XNI_FALSE, ROUND_JOIN_THRESHOLD);
    }
  }
...
dasher_lineTo(Pipeline* pipeline, jint x1, jint y1) {
  ...
  while (1) {
    ...
    if (pipeline->dasher_symmetric) {
      l = (jint)(((jlong)piscesmath_ihypot(lx, ly)*65536)/pipeline->dasher_ldet);
    } else{
    ...
    t = ((jlong)d << 16)/l;
    xsplit = pipeline->dasher_x0 + (jint)(t*(x1 - pipeline->dasher_x0) >> 16);
    ysplit = pipeline->dasher_y0 + (jint)(t*(y1 - pipeline->dasher_y0) >> 16);
...
flattener_quadToHelper(Pipeline* pipeline, 
    jint x1, jint y1, jint x2, jint y2) {
  if (flattener_flatEnough_3(pipeline, 
        pipeline->flattener_x0, pipeline->flattener_y0, x1, y1, x2, y2)) {
    pipeline->flattener_next_lineTo(pipeline, x1, y1);
...
stroker_finish(Pipeline* pipeline) {
  ...
  if (capStyle == CAP_ROUND) {
    stroker_drawRoundJoin(pipeline, pipeline->stroker_x0, pipeline->stroker_y0, 
        pipeline->stroker_omx, pipeline->stroker_omy, 
        -pipeline->stroker_omx, -pipeline->stroker_omy, 
        1, XNI_FALSE, XNI_FALSE, ROUND_JOIN_THRESHOLD);
  ...
  if (capStyle == CAP_ROUND) {
    stroker_drawRoundJoin(pipeline, pipeline->stroker_sx0, pipeline->stroker_sy0,
      -pipeline->stroker_mx0, -pipeline->stroker_my0, 
      pipeline->stroker_mx0, pipeline->stroker_my0, 
      1, XNI_FALSE, XNI_FALSE, ROUND_JOIN_THRESHOLD);
  } else if (capStyle == CAP_SQUARE) {
