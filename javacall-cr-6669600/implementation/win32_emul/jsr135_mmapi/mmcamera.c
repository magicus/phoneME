    ...
    if (JAVACALL_TRUE == _grabFrame) {
        ...
        if (_pFrameBuffer && _pEncodingBuffer) {
            ...
            javanotify_on_media_notification(JAVACALL_EVENT_MEDIA_SNAPSHOT_FINISHED,
                                             _isolateId, _playerId, JAVACALL_OK, 0);
...
#define DEFAULT_PREVIEW_RATE    150 /* ms unit => Increase this value to optimize performance */
    ...
    JC_MM_DEBUG_PRINT4("[camera] capCreateCaptureWindow %d %d %d %d\n", x, y, w, h);

    pHandle->hCapWnd = capCreateCaptureWindow(_T("Sun_Java_Cap_Window"), 
                   wsVisible | WS_CHILD | WS_CLIPSIBLINGS, 
                   x + X_SCREEN_OFFSET, y + Y_SCREEN_OFFSET + TOP_BAR_HEIGHT, w, h, GET_MCIWND_HWND(), 0xffff);

    JC_MM_DEBUG_PRINT1("[camera] capCreateCaptureWindow %d\n", pHandle->hCapWnd);

    if (pHandle->hCapWnd) {
        ret = capDriverConnect(pHandle->hCapWnd, DEFAULT_CAPTURE_DRIVER);
        if (FALSE == ret) {
            JC_MM_DEBUG_PRINT("[camera] capDriverConnect fail - is there camera attached?\n");
            DestroyWindow(pHandle->hCapWnd);
            pHandle->hCapWnd = NULL;
            return NULL;
        }
...
static javacall_result camera_start(javacall_handle handle)
{
    ...
    JC_MM_DEBUG_PRINT2("[camera] camera_start %d %d\n", pHandle->hCapWnd, pHandle->visible);
...
static javacall_result camera_stop(javacall_handle handle)
{
    ...
    JC_MM_DEBUG_PRINT1("[camera] camera_stop %d %d\n", pHandle->hCapWnd);
 ...
 * Don't do anything
 */
static javacall_result camera_get_time(javacall_handle handle, long* ms)
{
    *ms = -1;
    return JAVACALL_OK;
}
 ...
 * Don't do anything
 */
static javacall_result camera_set_time(javacall_handle handle, long* ms)
{
    *ms = -1;
    return JAVACALL_OK;
}
 ...
 * Don't do anything
 */
static javacall_result camera_get_duration(javacall_handle handle, long* ms)
{
    *ms = -1;
    return JAVACALL_OK;
}
...
static javacall_result camera_set_video_size(javacall_handle handle, long width, long height)
{
    ...
    JC_MM_DEBUG_PRINT2("[camera] camera_set_video_size %d %d\n", width, height);
...
static javacall_result camera_get_video_size(javacall_handle handle, long* width, long* height)
{
    ...
    if (width && height) {
      JC_MM_DEBUG_PRINT2("[camera] camera_get_video_size %d %d\n", *width, *height);
    }
...
static javacall_result camera_set_video_location(javacall_handle handle, long x, long y, long w, long h)
{
    ...
    JC_MM_DEBUG_PRINT4("[camera] camera_set_video_location %d %d %d %d\n", x, y, w, h);
...
static javacall_result camera_set_video_visible(javacall_handle handle, javacall_bool visible)
{
    ...
    JC_MM_DEBUG_PRINT2("[camera] camera_set_video_visible %d %d\n", handle, visible);
