...
# Generate 'nativeProcessList'
$(CVM_NATIVE_PROCESS_DIR):
	$(AT) echo creating $@ ...
	$(AT) mkdir -p $@
	$(AT) echo copying native_process.h ...
	$(AT) cp $(CVM_TOP)/src/portlibs/posix/native_process.h $@

$(CVM_NATIVE_PROCESSES):: $(CVM_NATIVE_PROCESS_DIR)
	$(AT) echo touching $@ ...
	$(AT) touch $@

$(CVM_NATIVE_PROCESS_DIR)/$(CVM_NATIVE_PROCESS_TBL): $(CVM_NATIVE_PROCESS_DIR) $(CVM_NATIVE_PROCESSES)
	$(AT) echo creating $@ ...
	$(AT) echo "/* This file is auto-generated by the CVM build process."> $@
	$(AT) echo "   Do not edit. */" >> $@
	$(AT) echo "#define NATIVE_PROCESS_TBL 1" >> $@
	$(AT) echo "#include \"native_process.h\"" >> $@
	$(AT) p_list=`cat $(CVM_NATIVE_PROCESSES)`; \
          if test ! "$${p_list}" = ""; then \
              for p in "$${p_list}"; do \
                  echo "DECL_PROCESS($${p})" >> $@; \
              done; \
          fi; \
          echo "ProcessRec nativeProcessList[] = {" >> $@; \
          if test ! "$${p_list}" = ""; then \
              for p in "$${p_list}"; do \
                  echo "PROCESS_ENTRY($${p})," >> $@; \
              done; \
          fi
	$(AT) echo "END_OF_PROCESS_LIST};" >> $@
endif
