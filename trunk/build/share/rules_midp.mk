...
initbuild_profile::
	...
	$(AT)$(MAKE) PCSL_PLATFORM=$(PCSL_PLATFORM) NETWORK_MODULE=$(NETWORK_MODULE)  PCSL_OUTPUT_DIR=$(PCSL_OUTPUT_DIR) GNU_TOOLS_BINDIR=$(GNU_TOOLS_BINDIR) -C $(PCSL_DIR)
...
$(MIDP_CLASSESZIP): force_midp_build
	$(AT)echo "Building MIDP classes ..."
	$(AT)$(MAKE) TARGET_VM=$(TARGET_VM) TARGET_CPU=$(TARGET_CPU) USE_DEBUG=$(USE_DEBUG) USE_SSL=$(USE_SSL) USE_RESTRICTED_CRYPTO=$(USE_RESTRICTED_CRYPTO) VERIFY_BUILD_ENV= CONFIGURATION_OVERRIDE=$(CONFIGURATION_OVERRIDE) USE_QT_FB=$(USE_QT_FB) USE_DIRECTFB=$(USE_DIRECTFB) USE_SSL=$(USE_SSL) USE_CONFIGURATOR=$(USE_CONFIGURATOR) USE_VERBOSE_MAKE=$(USE_VERBOSE_MAKE) PCSL_PLATFORM=$(PCSL_PLATFORM) GNU_TOOLS_BINDIR=$(GNU_TOOLS_BINDIR) rom -C $(MIDP_DIR)/build/linux_fb_gcc
...
$(RUNMIDLET): force_midp_build
	...
	$(AT)$(MAKE) TARGET_VM=$(TARGET_VM) TARGET_CPU=$(TARGET_CPU) USE_DEBUG=$(USE_DEBUG) USE_SSL=$(USE_SSL) USE_RESTRICTED_CRYPTO=$(USE_RESTRICTED_CRYPTO) VERIFY_BUILD_ENV= CONFIGURATION_OVERRIDE=$(CONFIGURATION_OVERRIDE) USE_QT_FB=$(USE_QT_FB) USE_DIRECTFB=$(USE_DIRECTFB) USE_SSL=$(USE_SSL) USE_CONFIGURATOR=$(USE_CONFIGURATOR) USE_VERBOSE_MAKE=$(USE_VERBOSE_MAKE) PCSL_PLATFORM=$(PCSL_PLATFORM) GNU_TOOLS_BINDIR=$(GNU_TOOLS_BINDIR) -C $(MIDP_DIR)/build/linux_fb_gcc
	$(AT)cp $@ $(CVM_BINDIR)
