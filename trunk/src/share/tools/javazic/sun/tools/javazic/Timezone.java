...
class Timezone {
	...
	for (int i = 0; i < transitions.size(); i++) {
	    ...
	    sum.update(((Long)transitions.get(i)).longValue()
		       + offset);
	    sum.update(offset);
	...
	for (int i = 0; i < (transitions.size() - 2); i++) { // don't remove the last one
	    if (((Long)transitions.get(i)).longValue()
		== ((Long)transitions.get(i+1)).longValue()) {
	...
	for (int i = 0; i < (transitions.size() - 2); i++) { // don't remove the last one
	    if (((Integer)offsets.get(i)).intValue()
		== ((Integer)offsets.get(i+1)).intValue()
		&& ((Integer)dstOffsets.get(i)).intValue()
		== ((Integer)dstOffsets.get(i+1)).intValue()) {
