...
public class GameDemo
    extends Demo implements KeyListener, FocusListener, Xlet {
    ...
    private static final Color COLOR_OUTSIDE = SUN_YELLOW;
    private static final Color COLOR_INSIDE = SUN_BLUE;
    private static final Color COLOR_ACTIVE = SUN_RED;
    private static final Color COLOR_SPARE = SUN_LIGHTBLUE;
    ...
    public GameDemo() {
        ...
        animator.start();
        pausedImage = loadImage(this, "paused.gif");
        abortedImage = loadImage(this, "aborted.gif");
        gameoverImage = loadImage(this, "gameover.gif");
        addKeyListener(this);
    ...
    public void initXlet(XletContext context) {
        try {
            System.out.println("***** XLET MESSAGE: initXlet: Jonix *****");
    ...
    public void startXlet() {
        System.out.println("***** XLET MESSAGE: startXlet: Jonix *****");
    ...
    public void pauseXlet() {
        System.out.println("***** XLET MESSAGE: pauseXlet: Jonix *****");
        if (!paused) {
            paused = true;
            long currentTime = System.currentTimeMillis();
    ...
    public void destroyXlet(boolean unconditional) {
        System.out.println("***** XLET MESSAGE: destroyXlet: Jonix *****");
    }
...
//        demo.show();
    ...
    public void paint(Graphics g) {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                ...
                g.fillRect(col * cellWidth, row * cellHeight, cellWidth,
                           cellHeight);
                if (cells[row][col] == SPARE) {
                    g.setColor(COLOR_BLANK);
                    g.drawRect(col * cellWidth, row * cellHeight, cellWidth - 1,
                               cellHeight - 1);
                }
            }
        }
        ...
        if (paused || aborted || gameover) {
            ...
            g.drawImage(image, demoWidth / 10, 4 * demoHeight / 9,
                        8 * demoWidth / 10, demoHeight / 9, this);
        }
        drawPercentageTime();
    }
    ...
    private void winner() {
        ...
        for (int i = 0; i < 300; i++) {
            ...
            g.drawString(string, demoWidth / 2 - fw / 2,
                         demoHeight / 2 + fontSize / 2);
            sleep(10);
        }
        winner = false;
    }
    ...
    class Animator
        ...
        public void run() {
            ...
            while (true) {
                ...
                while (paused || aborted || halted || gameover ||
                       showHelpStrings || showHighScores);
            }
        }
    }
    ...
    class Player {
        ...
        public void move() {
            ...
            if (stopped || paused || aborted || halted || gameover ||
                showHelpStrings || showHighScores) {
                return;
            }
            int oldRow = row;
            ...
            if (row != oldRow || col != oldCol) {
                ...
                g.fillRect(oldCol * cellWidth, oldRow * cellHeight, cellWidth,
                           cellHeight);
                if (cells[oldRow][oldCol] == SPARE) {
                    g.setColor(COLOR_BLANK);
                    g.drawRect(oldCol * cellWidth, oldRow * cellHeight,
    ...
    class Enemy {
        ...
        public void move() {
            if (paused || aborted || halted || crashed || gameover ||
                showHelpStrings || showHighScores) {
                return;
            }
            int oldRow = row;
