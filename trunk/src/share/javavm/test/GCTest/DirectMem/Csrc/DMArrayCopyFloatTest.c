/*
 * @(#)DMArrayCopyFloatTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayCopyFloatTest.h"


JNIEXPORT void JNICALL
Java_DMArrayCopyFloatTest_nSetArray(JNIEnv *env, jobject obj, 
                                 jfloatArray srcArray, jfloatArray dstArray)
{
   jint i;
   jobject lock;
   jfieldID iFid, oFid;
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      CVMD_arrayCopyFloat(dSrcFloatArray, 0, dDstFloatArray, 0, CVMD_arrayGetLength(dDstFloatArray));

      if(ee->barrier == W_BARRIER_FLOAT)
         printf("PASS: DMArrayCopyFloatTest, Write Barrier Float was invoked\n");
      else
         printf("FAIL: DMArrayCopyFloatTest, Write Barrier Float was not invoked\n");

      printf("\n");
       
      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayCopyFloatTest, Gc did not happen in the gc unsafe section\n");
      else
         printf("FAIL: DMArrayCopyFloatTest, Gc happened in the gc unsafe section\n");
