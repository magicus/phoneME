			  ...
			  JDIMENSION in_rows_avail)
{
  ...
  while (mainp->cur_iMCU_row < cinfo->total_iMCU_rows) {
    ...
    /* We did finish the row.  Undo the suspension if a previous
     * call is suspended; then mark the main buffer empty.
     */
    if (mainp->suspended) {
      (*in_row_ctr)++;
      mainp->suspended = FALSE;
...
GLOBAL(void)
jm_jinit_c_main_controller (j_compress_ptr cinfo, boolean need_full_buffer)
{
  my_main_ptr mainp;
  int ci;
  jpeg_component_info *compptr;
