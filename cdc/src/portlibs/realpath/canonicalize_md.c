/*
 * @(#)canonicalize_md.c	1.47 06/10/10
 *
...
canonicalize(const char *original, char *resolved, int len)
{
	...
	/* Invalid characters are in the original path, so remove names from 
	   the end until either some subpath works or we run out of names */
	char *p, *end, *r = NULL;
