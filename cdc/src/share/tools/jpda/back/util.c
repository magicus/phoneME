...
util_initialize(JNIEnv *env)
{
    WITH_LOCAL_REFS(env, 7) {
        ...
        gdata->threadCurrentThread =
          getStaticMethod(env, gdata->threadClass, "currentThread", 
                    "()Ljava/lang/Thread;");
        ...
        gdata->property_java_library_path   
...
classInstances(jclass klass, ObjectBatch *instances, int maxInstances)
{
    ...
    if ( maxInstances < 0 || instances == NULL || klass == NULL ) {
...
objectReferrers(jobject obj, ObjectBatch *referrers, int maxObjects)
{
    ...
    /* Check interface assumptions */
    if ( obj == NULL || referrers == NULL || maxObjects < 0 ) {
        return AGENT_ERROR_ILLEGAL_ARGUMENT;
    }
