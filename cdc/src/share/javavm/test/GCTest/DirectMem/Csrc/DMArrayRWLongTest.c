/*
 * @(#)DMArrayRWLongTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayRWLongTest.h"


JNIEXPORT void JNICALL
Java_DMArrayRWLongTest_nSetArray(JNIEnv *env, jobject obj, 
                              jlongArray dstArray, jlongArray srcArray, jint arrayLength)
{
   jint i;
   jlong val;
   jobject lock;
   ...
   CVMD_gcUnsafeExec( ee, {
      CVMArrayOfLong* directSrcArray = (CVMArrayOfLong *)CVMID_icellDirect(ee, srcArray);
      CVMArrayOfLong* directDstArray = (CVMArrayOfLong *)CVMID_icellDirect(ee, dstArray);
      jint rBarrier = 0;
      jint wBarrier = 0;
     ...
     for(i=0; i<arrayLength; i++) {
      CVMD_arrayReadLong(directSrcArray, i, val);
      if(ee->barrier == R_BARRIER_64)
         rBarrier++;
      CVMD_arrayWriteLong(directDstArray, i, val);
      if(ee->barrier == W_BARRIER_64)
         wBarrier++;
     }

      if(rBarrier == arrayLength && wBarrier == arrayLength)
         printf("PASS: DMArrayRWLongTest, Read & Write Barrier Long were invoked\n");
      else
         printf("FAIL: DMArrayRWLongTest, Read & Write Barrier Long were not invoked\n");

      printf("\n");
       
      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayRWLongTest, Gc did not happen in the gc unsafe section\n");
      else
         printf("FAIL: DMArrayRWLongTest, Gc happened in the gc unsafe section\n");
   } );
}
