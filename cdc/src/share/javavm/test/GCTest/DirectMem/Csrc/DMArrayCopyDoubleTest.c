/*
 * @(#)DMArrayCopyDoubleTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayCopyDoubleTest.h"


JNIEXPORT void JNICALL
Java_DMArrayCopyDoubleTest_nSetArray(JNIEnv *env, jobject obj, 
                                jdoubleArray srcArray, jdoubleArray dstArray)
{
   jint i;
   jobject lock;
   jfieldID iFid, oFid;
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      CVMD_arrayCopyDouble(dSrcDoubleArray, 0, dDstDoubleArray, 0, CVMD_arrayGetLength(dDstDoubleArray));

      if(ee->barrier == W_BARRIER_64)
         printf("PASS: DMArrayCopyDoubleTest, Write Barrier 64 was invoked\n");
      else {
         printf("FAIL: DMArrayCopyDoubleTest, Write Barrier 64 was not invoked\n");
         printf("barrier is %d \n", ee->barrier);
      }
      ...
      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayCopyDoubleTest, Gc did not happen in the gc unsafe section\n");
      else
         printf("FAIL: DMArrayCopyDoubleTest, Gc happened in the gc unsafe section\n");
