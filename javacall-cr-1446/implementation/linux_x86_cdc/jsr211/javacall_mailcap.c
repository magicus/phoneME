...
javacall_result javacall_chapi_init_registry(void){
	...
	while (1){
		...
		if (getenv("HOME")){
			fpos = sprintf(buf,"%s/.mailcap",getenv("MAILCAPS"));
			sprintf(&buf[fpos],"/.mailcap");
			if (file_exists(buf)) break;
		}
...
javacall_result javacall_chapi_enum_handlers_by_id_prefix(javacall_const_utf16_string full_handler_id, int* pos_id, /*OUT*/ javacall_utf16*  buffer, int* length)
{
	int result;
	int index=*pos_id;
	int prefix_len;
...
#ifdef DEBUG_OUTPUT
	wprintf(L"JAVACALL::javacall_chapi_enum_handlers_by_id_prefix(%s,%d)\n",full_handler_id,*pos_id);
#endif
	...
	while (index < g_handler_infos_used){
		prefix_len = javautil_str_wcslen(g_handler_infos[index]->handler_id);
		if (!javautil_str_wcsncmp(full_handler_id,g_handler_infos[index]->handler_id,prefix_len)){
			result=get_id(g_handler_infos[index],buffer,length);
			if (!result){
				*pos_id = index+1;
			}
