             ...
             PNG_ENCODER_INPUT_COLOR_FORMAT colorFormat)
{
    ...
    for (i = 0; i < (int)sizeof(png_magic); i++)
	...
	for (j = 0; j < ROWS_PER_GROUP; j++) {
	    ...
	    /* write pixels */
	    for (k = 0; k < width; k++) {
		if (colorFormat == PNG_ENCODER_COLOR_BGRX) {
		...
		} else /* if (colorFormat == PNG_ENCODER_COLOR_XRGB) */ {
		    zcrc = adler32(zcrc, &(input[1]), 1);
		    writebytecrc(&enc, input[1]);
		    zcrc = adler32(zcrc, &(input[2]), 1);
		    writebytecrc(&enc, input[2]);
