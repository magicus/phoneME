...
public final class MIDPVideoRenderer extends VideoRenderer
    ...
    public byte[] getSnapshot(String imageType)
        ...
        synchronized (this) {
            ...
            LocatorParser lp;
            // Set the defaults
            String enc = LocatorParser.S_ENCODING_JPEG;
            int encType = ImageEncoder.CONVERT_RGB_TO_JPEG; // JPEG is the default
            ...
            // If the image type is specified, check the parameters
            if (imageType != null) {
                ...
                // JPEG ?
                if (enc.equals(LocatorParser.S_ENCODING_JPEG)) {

                    video_type = LocatorParser.S_TYPE_JFIF;
                    quality = 80;
                
                    while ((key = lp.getParameter()) != null) {
                        value = lp.getValue();
                        try {
                            if (key.equals(lp.S_WIDTH))
                                width = Integer.parseInt(value);
                            else if (key.equals(lp.S_HEIGHT))
                                height = Integer.parseInt(value);
                            else if (key.equals(lp.S_TYPE))
                                video_type = value;
                            else if (key.equals(lp.S_PROGRESSIVE))
                                progressive = value.equals(lp.S_TRUE);
                            else if (key.equals(lp.S_INTERLACED))
                                interlaced = value.equals(lp.S_TRUE);
                            else if (key.equals(lp.S_QUALITY))
                                quality = Integer.parseInt(value);
                            else if (key.equals(lp.S_FPS) ||
                                     key.equals(lp.S_COLORS))
                                    ;
                            else
                    ...
                    // Do we accept these parameters?
                    if (lp.hasMore() || !video_type.equals(lp.S_TYPE_JFIF) ||
                        width != videoWidth || 
                        height != videoHeight || 
                ...
                // PNG
                else if (enc.equals(LocatorParser.S_ENCODING_PNG)) {
                    encType = ImageEncoder.CONVERT_RGB_TO_PNG;
                    while ((key = lp.getParameter()) != null) {
                        value = lp.getValue();
                        try {
                            if (key.equals(lp.S_WIDTH))
                                ...
                                height = Integer.parseInt(value);
                            else if (key.equals(lp.S_COLORS))
                                colors = value;
                            else if (key.equals(lp.S_FPS)
                                     || key.equals(lp.S_QUALITY))
                                    ;
                            else
                                throw new MediaException("Illegal option");
                        } catch (NumberFormatException nfe) {
                    ...
                    // Do we accept these parameters?
                    if (lp.hasMore() || !colors.equals("rgb888") ||
                        width != videoWidth || height != videoHeight || progressive || interlaced) {
            ...
            if (pngData != null) {
                // If it's PNG data coming into MIDPVideoRenderer
                ...
                snapLength = ImageEncoder.RGBIntCompress(
                        tempRGBData, 
                        videoWidth, videoHeight, 
                        quality,
                        tempSnapData, encType);
            } else if (rgbData != null) {
                // If it's RGB data coming into MIDPVideoRenderer
                snapLength = ImageEncoder.RGBIntCompress(
                        rgbData, 
                        videoWidth, videoHeight, 
                        quality, // quality parameter unused in case of PNG
                        tempSnapData, encType);
                if (snapLength <= 0) {
                ...
                for (int i = 0; i < videoWidth * videoHeight * 4; i++)
                    blank[i] = 0;
                snapLength = ImageEncoder.RGBByteCompress(
                        blank,
                        videoWidth, videoHeight, quality, 
                        tempSnapData, encType);
            }
            ...
            System.arraycopy(tempSnapData, 0,
                             snap, 0,
                             snapLength);
            return snap;
        }
