 ...
 * data buffer
 * IMPL_NOTE:   binaryBuffer must be aligned to 4 bytes and contain 
 *              the subsystem id in first four bytes
...
javacall_result javacall_event_receive(
    ...
    EventQueue *q;
    int totalRead = 0;
    // extract the subsystem id (JSRno) from the message
    // IMPL_NOTE: binaryBuffer must be aligned to javacall_int32
    javacall_int32 *event = (javacall_int32 *) binaryBuffer;

    if (binaryBuffer == NULL || binaryBufferMaxLen <= sizeof (javacall_int32)) {
        return JAVACALL_FAIL;
    }
    q = find_event_queue(event[0]);
    if (q == NULL) {
        return JAVACALL_FAIL;
    }
    ...
    while (ok) {
        ...
        if (ok) {
            totalRead = dequeueEventMessage(q, (unsigned char*) &event[1], 
        ...
        if (0 == totalRead) {
            ok = checkForEvents(q, timeTowaitInMillisec);
        } else {
            break;
        }
    }
...
javacall_result javacall_event_send(unsigned char* binaryBuffer,
    ...
    if (ok) {
        enqueueEventMessage(q, (unsigned char*) &event[1], 
