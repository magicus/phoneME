...
public class IxcInputStream extends ObjectInputStream {
   ...
   IxcInputStream(InputStream in, XletContext context, boolean isAppManager) 
       ...
       * for the central CDCAmsIxcRegistry's input stream.
   ...
   protected Object resolveObject(Object obj) 
      ...
      if (nextObject instanceof Remote) { 
         ...
         if (eo != null && eo.getRemoteRef().equals(remoteRef)) {
            ...
            reportImportToCDCAmsIxcRegistry((StubObject)nextObject, context);
         }
      }
    ...
    void reportImportToCDCAmsIxcRegistry(StubObject r,
                                   XletContext importer) {
                                                                                
       // Report to the CDCAmsIxcRegistry about this new import if
       // the connection is made not with
       // CDCAmsIxcRegistry itself but with some other client VM.

       IxcRegistry registry = IxcRegistry.getRegistry(importer);
                                                                                
       if (registry instanceof MvmIxcRegistryImpl) {
          ((MvmIxcRegistryImpl)registry).amHandler.notifyObjectImport(
