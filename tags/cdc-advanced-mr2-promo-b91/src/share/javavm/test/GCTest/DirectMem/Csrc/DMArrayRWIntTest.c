/*
 * @(#)DMArrayRWIntTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayRWIntTest.h"


JNIEXPORT void JNICALL
Java_DMArrayRWIntTest_nSetArray(JNIEnv *env, jobject obj, 
                              jintArray dstArray, jintArray srcArray, jint arrayLength)
{
   jint i, val;
   jobject lock;
   jfieldID iFid, oFid;
   ...
   CVMD_gcUnsafeExec( ee, {
      CVMArrayOfInt* directSrcArray = (CVMArrayOfInt *)CVMID_icellDirect(ee, srcArray);
      CVMArrayOfInt* directDstArray = (CVMArrayOfInt *)CVMID_icellDirect(ee, dstArray);
      jint rBarrier = 0;
      jint wBarrier = 0;
     ...
     for(i=0; i<arrayLength; i++) {
      CVMD_arrayReadInt(directSrcArray, i, val);
      if(ee->barrier == R_BARRIER_INT)
         rBarrier++;
      CVMD_arrayWriteInt(directDstArray, i, val);
      if(ee->barrier == W_BARRIER_INT)
         wBarrier++;
     }

      if(rBarrier == arrayLength && wBarrier == arrayLength)
         printf("PASS: DMArrayRWIntTest, Read & Write Barrier Int were invoked\n");
      else
         printf("FAIL: DMArrayRWIntTest, Read & Write Barrier Int were not invoked\n");

      printf("\n");
       
      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayRWIntTest, Gc did not happen in the gc unsafe section\n");
      else
         printf("FAIL: DMArrayRWIntTest, Gc happened in the gc unsafe section\n");
   } );
}
