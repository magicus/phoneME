...
public class Main {
    ...
    private URL parseURL(String url) throws ParseException {
        ...
        try {
            if (url.indexOf(':') <= 1) {
                ...
                // win32 machine
                u = new File(System.getProperty("user.dir"), url).toURL();
            } else {
                if (url.startsWith("file:") &&
                    ...
                    if (fname.length() > 0) {
                        u = new File(System.getProperty("user.dir"),
                                    fname).toURL();
                    } else {
                        u = new URL(url);
                    }
    ...
    private void init() {
        ...
        avProps.put("acl.read", "+");
        avProps.put("acl.read.default", "");
        avProps.put("acl.write", "+");
        avProps.put("acl.write.default", "");
        ...
        // TODO: Create and install a socket factory!
    }
