/*
 * @(#)IDFieldRWIntTest.c	1.8 06/10/10
 *
...
#include <stdio.h>
#include "IDFieldRWIntTest.h"

JNIEXPORT jintArray JNICALL 
Java_IDFieldRWIntTest_nGetValues (JNIEnv *env, jobject obj)
{
   jint i, j, var;
   jint numFields = 0;
   jclass clazz;
      ...
      CVMD_gcSafeExec(ee, {
         ...
         for(i=0, j=0; i< CVMcbFieldCount(cb); i++) {
            ...
            if(!CVMfbIs(fb, STATIC)) {
               ...
               CVMID_fieldReadInt(ee, (CVMObjectICell*) obj, objOffset, var);
               CVMID_arrayWriteInt(ee, (CVMArrayOfIntICell*) intArray, i, var);
               ++j;
            }
         }
      });
...
Java_IDFieldRWIntTest_nSetValues(JNIEnv *env, jobject obj, 
                       jint v1, jint v2, jint v3)
{
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      CVMD_gcSafeExec(ee, {
         CVMID_fieldWriteInt(ee, (CVMObjectICell*) obj, objOffset[0], v1);
         CVMID_fieldWriteInt(ee, (CVMObjectICell*) obj, objOffset[1], v2);
         CVMID_fieldWriteInt(ee, (CVMObjectICell*) obj, objOffset[2], v3);
      });
