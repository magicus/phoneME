...
public class X500Name implements GeneralNameInterface, Principal {
	 ...
	 */
        StringBuffer fullname = new StringBuffer(48);
        for (int i = names.length - 1; i >= 0; i--) {
	    if (i < names.length - 1) {
		fullname.append(',');
	    }
            fullname.append(names[i].toRFC2253String());
        }
	rfc2253Dn = new String(fullname);
	return rfc2253Dn;
    }
	...
	canonicalDn = new String(fullname);
	return canonicalDn;
    }
     ...
     *
     * This method can parse 1779 or 2253 DNs and non-standard 2459 keywords.
     */
    private void parseDN(String input) throws IOException {
	 ...
	 */
	int numElem = dnVector.size();
	names = new RDN[numElem];
	Enumeration e = dnVector.elements();
	int i = numElem - 1;

	while (e.hasMoreElements()) {
	    names[i--] = (RDN)e.nextElement();
	}
    }
     ...
     *
     * Uses RFC 1779 syntax (i.e. little-endian, comma separators)
     * Only valid keywords from RFC 1779 are used.
     */
    private void generateRFC1779DN() {
	if (names.length == 1) {
	    rfc1779Dn = names[0].toRFC1779String();
	    return;
	}
	...
	if (names != null) {
		...
		sb.append(names[i].toRFC1779String());
	    }
	}
	rfc1779Dn = sb.toString();
    }
     ...
     * Includes all those specified in RFC2459 as MUST or SHOULD
     * be recognized
     */
    ...
    /*
     * Domain component OID from RFC 1274, RFC 2247, RFC 2459
     */
    ...
    static {
        ...
        try {
	    Object[] result = (Object[])AccessController.doPrivileged(pa);
