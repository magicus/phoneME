/*
 * @(#)DMArrayCopyRefTest.c	1.7 06/10/10
 *
...
#include <stdio.h>
#include "DMArrayCopyRefTest.h"


JNIEXPORT void JNICALL
Java_DMArrayCopyRefTest_nSetArray(JNIEnv *env, jobject obj, 
                                  jobjectArray srcArray, jobjectArray dstArray)
{
   jint i;
   jobject lock;
   jfieldID iFid, oFid;
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      CVMD_arrayCopyRef(dSrcRefArray, 0, dDstRefArray, 0, CVMD_arrayGetLength(dDstRefArray));

      if(ee->barrier == W_BARRIER_REF)
         printf("PASS: DMArrayCopyRefTest, Write Barrier Ref was invoked\n");
      else
         printf("FAIL: DMArrayCopyRefTest, Write Barrier Ref was not invoked\n");

      printf("\n");
       
      if(CVMfbStaticField(ee, iFid).i == -1)
         printf("PASS: DMArrayCopyRefTest, Gc did not happen in the gc unsafe section\n");
      else
         printf("FAIL: DMArrayCopyRefTest, Gc happened in the gc unsafe section\n");
