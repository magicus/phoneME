/*
 * @(#)DMFieldRW32Test.c	1.9 06/10/10
 *
...
#include <stdio.h>
#include "DMFieldRW32Test.h"

JNIEXPORT jintArray JNICALL 
Java_DMFieldRW32Test_nGetValues (JNIEnv *env, jobject obj) 
{
   jint i, j, count = 0;
   jint numFields = 0;
   jintArray intArray;
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      for(i=0, j=0; i< CVMcbFieldCount(cb); i++) {
         ...
         if(!CVMfbIs(fb, STATIC)) {
            ...
            CVMD_fieldRead32(directObject, objOffset, var);

            if(ee->barrier == R_BARRIER_INT)
               count++;

            CVMD_arrayWriteInt(directIntArray, i, var.i);
            ++j;
         }
      }
	...
	printf("PASS: DMFieldRW32Test:Read, Gc did not happen in the gc unsafe section\n");
      else
	printf("FAIL: DMFieldRW32Test:Read, Gc happened in the gc unsafe section\n");
   });
...
Java_DMFieldRW32Test_nSetValues(JNIEnv *env, jobject obj, 
                       jint v1, jint v2, jint v3)
{
   ...
   CVMD_gcUnsafeExec( ee, {
      ...
      var.i = v1;
      CVMD_fieldWrite32(directObject, objOffset[0], var);
      if(ee->barrier == W_BARRIER_INT)
         count++;
      var.i = v2;
      CVMD_fieldWrite32(directObject, objOffset[1], var);
      if(ee->barrier == W_BARRIER_INT)
         count++;
      var.i = v3;
      CVMD_fieldWrite32(directObject, objOffset[2], var);
      if(ee->barrier == W_BARRIER_INT)
         count++;
      ...
      if(CVMfbStaticField(ee, iFid).i == -1)
	printf("PASS: DMFieldRW32Test:Write, Gc did not happen in the gc unsafe section\n");
      else
	printf("FAIL: DMFieldRW32Test:Write, Gc happened in the gc unsafe section\n");
   } );
}
