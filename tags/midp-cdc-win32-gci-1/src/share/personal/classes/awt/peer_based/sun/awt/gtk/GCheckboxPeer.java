/*
 * @(#)GCheckboxPeer.java	1.15 06/10/10
 *
...
class GCheckboxPeer extends GComponentPeer implements CheckboxPeer {
    ...
    /** Creates a new GCheckboxPeer. */

    GCheckboxPeer (GToolkit toolkit, Checkbox target) {
    ...
    private native void create(boolean radio);
    ...
    public void setLabel(String label) {
        setLabelNative(stringToNulMultiByte(label));
    }

    private native void setLabelNative(byte[] label);
    ...
    private void postItemEvent(boolean state) {
        ...
        cb.setState(state);
        // For some reason Java 2 only sends selected events is the checkbox is in a chekbox group.
        // I don't know why this is but implemented this anyway to be consistent.
		
        if (cbg == null || state)
            GToolkit.postEvent(new ItemEvent(cb, ItemEvent.ITEM_STATE_CHANGED,
                    cb.getLabel(),
