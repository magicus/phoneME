...
class ThreadGroup {
    ...
    */
	...
	synchronized (this) {
		...
		if (groups[i] == g) {
		    if (i < --ngroups) { // if not the last active entry
			// slide remaining active entries 1 slot to left
		        System.arraycopy(groups, i + 1, groups, i, ngroups - i);
		    }
		    // Clear vacated entry for GC.
		    groups[ngroups] = null;
		    break;
		}
	    }
	...
	synchronized (this) {
		...
		return;
            ...
            for (int i = 0 ; i < nthreads ; i++) {
		if (threads[i] == t) {
                    if (i < --nthreads) { // if not the last active entry
                       // slide remaining active entries 1 slot to left
		       System.arraycopy(threads, i + 1, threads, i, nthreads - i);
                    }
		    // Clear the vacated entry
