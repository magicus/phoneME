...
CVMJITgetMassagedIROpcode(CVMJITCompilationContext *con, CVMJITIRNode* ip)
{
    ...
    case CVMJIT_FETCH:
       /* SVMC_JIT */
#ifndef CVM_JIT_USE_FP_HARDWARE
    case CVMJIT_INVOKE:
#endif
	...
	case CVM_TYPEID_BYTE:
	case CVM_TYPEID_BOOLEAN:
	    /* converted by the IR generator to INT. */
	    CVMassert(CVM_FALSE);
	case CVMJIT_TYPEID_32BITS:
	    fulltag = fulltag & ~CVMJIT_TYPE_MASK;
	    return fulltag | CVM_TYPEID_INT;
...
#endif
#if 0
    case CVMJIT_FOR_EFFECT:
#endif
	/* Keep "untyped" SEQUENCE, FOR_EFFECT and END_INLINING as is. */
    ...
    case CVMJIT_CONST_STRING_OBJECT:
        return CVMJIT_ENCODE_CONST_STRING_OBJECT;
    case CVMJIT_CONST_STATIC_FIELD_ADDRESS:
        return CVMJIT_ENCODE_CONST_STATIC_FIELD_ADDRESS;
